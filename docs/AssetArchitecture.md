...
FormatExporter는 Asset을 직접 입력으로 받지 않고, Domain State 전체를 ExportDocument DTO로 변환한 뒤 그 DTO를 PDF/PNG/PPTX 등 실제 파일 포맷으로 변환한다. 상세 파이프라인 문서는 별도로 두지 않았다 — ImportPipeline.md에 대응하는 ExportArchitecture.md 작성을 권장한다.
