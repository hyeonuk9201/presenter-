...
# ~~진행 중인 설계 변경 (2026-07-04)~~ → 해소됨 (9-19에서 구현 완료)

이 절이 예고했던 Section 소속 모델 전환(`FutureEditor.md`의
`D-Editor-4` — `Page.sectionId` 기반 명시적 소속, D-Editor-1/D-Editor-3
대체)은 **9-19에서 코드에 반영 완료됐다** (스키마 v1→v2 마이그레이션
포함, `TODO.md`의 "Section Migration TODO" 참조). 이 절은 한동안 "아직
미구현"이라는 문구 그대로 남아 있어 최신 상태를 오도할 수 있었으므로
2026-07-11 문서 정리(9-38)에서 해소 표시했다.

당시 함께 언급됐던 Research 단계 아이디어(Flow View/Content Browser
이중 뷰)는 여전히 Research 단계다 — `TODO.md`의 P3 항목 참조.
...
## 다음 단계 진입 시 주의사항 (Step6 당시 기록 — 2026-07-11 문서 정리에서 상태 주석 추가)

> **⚠ 이 블록은 Step6(2026-06-27) 진행 중에 쓰인 기록이며, 아래 1~4번은
> 이후 세션에서 전부 해소됐다** (9-38에서 확인). 1번: `isValidPage()`는
> 현재 `['text', 'image', 'video']`를 허용하고 image/video의 `mediaId`
> 필수 검증까지 포함한다. 2번: `Page.js`의 필드명은 `mediaId`로
> `CommandBus.js`와 일치한다. 3번: output.html의 독립 resolve는 구현
> 완료됐고, **캐시 미스 시 동작도 "미정"이 아니라 결정·구현됐다** —
> resolve가 끝날 때까지 현재 화면을 유지하고 로딩 UI는 넣지 않는다
> (2026-06-27 합의, `output.html` 주석에 명문화. 라이브 화면에서 빈
> 화면/깜빡임이 구화면 유지보다 더 거슬린다는 판단 — D-009와 같은 결).
> 4번: 업로드 UI 구현 완료. **5번(GC 범위 밖)만 여전히 유효하다.**
> 원문은 당시 판단 기록으로 그대로 남긴다.
...
## 9-34. 스타일 프리셋 — 업데이트(덮어쓰기) (2026-07-11, TODO.md 로드맵의 P2 항목 구현)

### 배경

9-33 이후 `TODO.md`의 "다음 후보(P2)" 세 항목(Transition/Auto Advance/
스타일 프리셋 업데이트) 중 리스크가 가장 낮은 항목을 선정했다. Transition/
Auto Advance는 `output.html`/Live 송출 타이밍에 직접 관여해 Core
Philosophy("편집보다 송출 안정성 우선", `Architecture.md`)상 신중해야 하는
반면, 스타일 프리셋 업데이트는 Editor/Library 레이어에 국한된 국소 변경이라
먼저 착수했다.

착수 전 `docs/DocumentationHierarchy.md`(2026-07-11 커밋, 아직 Draft)의
판단 순서를 그대로 적용했다 — Decision 충돌 여부만 `Decisions.md` 기준으로
확인했다. `TODO.md`에 이미 ⚠로 표시돼 있던 `D-023` Non-goal("스타일 변경
시 기존에 적용됐던 Page를 일괄 업데이트하는 기능 지원 안 함")과 충돌하는지
확인한 결과: `Page`는 애초에 `presetId` 같은 참조 필드를 갖지 않으므로
(`D-023`), "프리셋 정의 자체의 서식 값만 교체"로 범위를 한정하면 이미
적용된 Page에 소급 반영될 코드 경로 자체가 구조적으로 존재할 수 없다 —
충돌 가능성이 아니라 애초에 충돌이 불가능한 설계였다는 것을 확인하고
착수했다.

### 구현

**`index.html`**: 프리셋 액션 행("적용"/"새로 저장"/"삭제")에 "덮어쓰기"
버튼을 추가했다. 클릭 시 현재 선택된 프리셋의 `id`/`name`은 그대로 두고
사이드바에 입력된 서식 값(정렬/굵기/크기/줄간격/색상/외곽선/그림자)만
교체해 `updateStylePreset()`을 호출한다 — 이 store 함수는 9-27부터 이미
존재했지만 호출부가 없어 죽은 코드였다. "적용"과 동일하게 값 복사
원칙(`D-023`)을 따르며, 새로운 커밋 경로를 만들지 않았다. 프리셋 미선택
시에는 안내 토스트만 띄우고 아무 일도 하지 않는다(기존 "적용"/"삭제"
버튼과 동일한 관례).

### 회귀 테스트 추가

`store/AppSettingsStore.test.js` 신규(5개) — 최초 실행 시 기본 프리셋
2개 시드, `addStylePreset`/`updateStylePreset`/`removeStylePreset`
CRUD, 그리고 **`updateStylePreset`이 대상 프리셋의 값만 바꾸고 다른
프리셋은 완전히 그대로 유지하는지**를 확인한다. 마지막으로 정적
검증 하나를 추가했다 — `AppSettingsStore.js` 소스에 `AppStore.js`/
`Presentation.js` import가 없는지 텍스트로 확인해, "이미 적용된 Page로
소급 반영되는 경로"가 코드 구조상 존재할 수 없음을 의존 방향 자체로
보장한다(런타임 동작이 아니라 구조 검증).

`npm test` 47개 전부 통과(기존 42 + AppSettingsStore 5).

### 브라우저 테스트 (Playwright, 자동화)

임시 스크립트로 검증 후 삭제(D-028 관행, 9-32부터 이어지는 방식).

- 덮어쓰기 버튼 존재 확인, 프리셋 미선택 시 안내 토스트
- "찬양 기본" 적용(fontSize 72) → 그 값으로 실제 Page 하나 생성
- 사이드바 값을 바꾸고 "덮어쓰기" → 성공 토스트
- **새로고침(모듈 재로드) 후에도 덮어쓴 값(fontSize 100, color #00ff00)이 유지됨**
- **다른 프리셋("설교 자막")은 영향받지 않고 원래 값(48) 유지**
- **핵심 확인: 덮어쓰기 이전에 이미 만들어둔 Page를 다시 선택하면 fontSize가 여전히 72다 — 소급 반영이 실제로 일어나지 않음을 브라우저에서 직접 확인**
- 콘솔 에러 0건
...
`index.html`(프리셋 액션에 "덮어쓰기" 버튼/핸들러 추가),
`store/AppSettingsStore.test.js`(신규), `docs/TODO.md`(항목 완료 처리,
아키텍처 원칙 충돌 항목 표에서 제거 — 충돌 아님을 확인했으므로).

### 다음 단계 진입 시 주의사항

1. **"덮어쓰기"는 이름을 바꾸지 않는다.** 프리셋 이름을 바꾸고 싶으면
   여전히 "삭제 후 새로 저장"해야 한다 — 이름 변경까지 포함한 완전한
   "편집" UI는 이번 범위 밖이다(TODO.md에 새 항목으로 등록하지 않음,
   필요성이 실사용으로 확인되면 그때 추가).
2. **덮어쓰기는 확인 절차 없이 즉시 반영된다.** Song 편집("저장" 버튼)과
   동일한 관례를 따른 것 — 삭제류(되돌릴 수 없는 제거)에만 2단계 확인을
   쓰고, 값 교체(되돌릴 수 있는 편집)에는 쓰지 않는다는 기존 패턴을
   그대로 유지했다.

## 9-35. Transition (2026-07-11, TODO.md 로드맵의 P2 항목 구현)

### 배경

9-34 이후 남은 P2 후보 두 개(Transition/Auto Advance) 중 리스크가 낮은
쪽을 선정했다. Auto Advance는 타이머가 스스로 다음 Page로 넘어가는 자동
동작이라 Live 송출 중 운영자가 의도하지 않은 시점에 화면이 바뀔 위험이
있는 반면, Transition은 "어떤 Page로 넘어가는가"는 그대로 두고 "어떻게
보이는가"만 바꾸는 시각 효과라 상대적으로 안전하다고 판단했다.

착수 전 `Decisions.md`를 확인했으나 Transition/애니메이션과 관련된
D-0XX는 없었다 — Decision 충돌 없음. `docs/DocumentationHierarchy.md`
(Draft)의 판단 순서를 그대로 적용했고, 이번에도 마찰 없이 적용됐다.

### 구현

**`output.html`**: `renderPage()`가 `page.transition.type`을 확인해
분기한다. `'fade'`이고 직전에 표시 중이던 슬라이드가 있으면
`crossfadeToSlide()`로 크로스페이드하고, 그 외(`'none'`/`'cut'`/
미인식 값/STANDBY 직후 첫 Page)는 기존과 동일한 즉시 교체(cut)를
유지한다 — 즉 지금까지의 기본 동작은 전혀 바뀌지 않는다.

크로스페이드는 새 슬라이드를 `position:absolute`로 이전 슬라이드 위에
opacity 0으로 겹쳐 놓고, 강제 리플로우 후 opacity를 1로 올려 CSS
transition이 실제로 재생되게 한 뒤, duration 후 이전 슬라이드를
제거한다. "이전 슬라이드가 무엇인가"는 DOM 조회(`container.querySelector`)
대신 `currentSlideEl`이라는 모듈 스코프 변수로 직접 추적한다 —
페이드가 끝나기 전에 다음 Page로 또 전환되는 경우(빠른 연속 클릭),
DOM에는 아직 이전 슬라이드가 남아있어 DOM 순서 기준 조회가 잘못된
슬라이드를 "이전 슬라이드"로 오인할 수 있기 때문이다. 각 크로스페이드
호출은 자신이 시작할 때의 `currentSlideEl`을 클로저로 캡처해 자기
책임의 슬라이드만 정리하고, 이미 더 최신 슬라이드로 바뀌었으면 건드리지
않는다.

STANDBY로 돌아갈 때(`showStandby()`)는 `currentSlideEl`을 `null`로
리셋한다 — STANDBY 자체는 페이드 대상에서 제외(범위 단순화, 아래
"다음 단계 진입 시 주의사항" 참조).

**`index.html`**: 사이드바에 "전환"(`#page-transition`, "즉시 전환"/
"페이드") select를 추가했다. 필드는 있었지만 사용자가 실제로 값을
바꿀 UI가 아예 없었다는 것을 뒤늦게 확인해서(Completion Criteria가
"Live Mode에서 시각적으로 확인"인데, 렌더러만 고쳐선 사용자가 값을
설정할 방법이 없어 확인 자체가 불가능했다) 함께 추가했다 — 렌더러
구현만으로는 이 TODO 항목이 실제로 "완료"될 수 없었다. "저장"
(UPDATE_PAGE)/"가사 추가"(ADD_PAGE) 두 커밋 경로 모두에 연결했고,
Page 선택 시 내용 로드 로직에도 반영했다. 미디어 Page(image/video)는
업로드 시점에는 기본값(`'none'`)으로 생성되고, 이후 "저장" 버튼으로
다른 스타일 필드(색상/크기 등)와 동일한 방식으로 조정 가능하다 —
기존 패턴 그대로.

**`domain/Page.js`**: `createBehaviorDefaults()` 위 주석을 갱신 —
"렌더러가 안 읽는다"는 설명이 이제 사실이 아니므로, output.html이
실제로 소비한다는 내용과 `ui/PreviewPanel.js`는 의도적으로 범위에서
뺐다는 이유(매 상태 변경마다 재렌더링되는 구조라 페이드를 넣으면
편집 중에도 계속 재생돼 오히려 방해가 됨)를 남겼다. 필드 구조 자체는
무수정.

### 회귀 테스트

`node:test`가 커버하지 않는 `output.html`/`index.html`의 embedded
script/렌더링 영역이라(D-028 범위 밖) Playwright(로컬 정적 서버 +
Chromium, 메인 탭과 Output 탭을 동시에 띄워 실제 `BroadcastChannel`
통신까지 검증)로 확인했다. 임시 스크립트는 검증 후 삭제(D-028 관행).

- 사이드바에 전환 select 존재 확인
- STANDBY → Page A(`'none'`): 슬라이드 1개, 즉시 표시(페이드 대상 없음)
- Page A → Page B(`'fade'`): 전환 도중(60ms 시점, duration 300ms 미만) 슬라이드 2개가 겹쳐 있음 → 완료 후(500ms) 슬라이드 1개, "Page A"는 사라지고 "Page B"만 표시
- Page B → Page A(`'none'`): 대기 없이(30ms) 즉시 슬라이드 1개로 교체(크로스페이드 없음)
- **빠른 연속 전환**(B→C, 첫 전환의 300ms가 끝나기 전에 다음 전환 트리거): 최종적으로 슬라이드가 정확히 1개만 남고("Page C"), 이전 슬라이드들이 leak되지 않음
- 콘솔 에러 0건(메인 탭/Output 탭 둘 다)

전체 9개 체크 전부 통과.

### 변경 파일

`output.html`(`renderPage()`/`crossfadeToSlide()`/`showStandby()`),
`index.html`(전환 select 추가 + "저장"/"가사 추가"/내용 로드 3곳
연결), `domain/Page.js`(주석 갱신, 필드 무수정), `docs/TODO.md`(항목
완료 처리).

### 다음 단계 진입 시 주의사항

1. **STANDBY 전환(CLEAR)은 항상 즉시(cut)다.** `'fade'` Page에서
   CLEAR로 넘어가도 페이드 아웃되지 않는다 — 의도적 범위 제한(Page의
   `transition`은 "다음 Page로 넘어올 때 어떻게 보이는가"에 대한
   설정이라고 해석했고, STANDBY는 Page가 아니므로 대상에서 뺐다).
   실사용 중 STANDBY 전환도 부드럽게 하고 싶다는 요청이 나오면 별도
   TODO 항목으로 다룬다.
2. **`ui/PreviewPanel.js`(에디터 내 미리보기)는 여전히 즉시 교체만
   한다.** 의도적 결정이며 위 "구현" 절 참조 — 매 상태 변경마다
   재렌더링되는 구조와 페이드가 안 맞는다.
3. **`'cut'` 타입은 `'none'`과 동일하게 즉시 교체로 처리된다.**
   `page.transition.type === 'fade'`가 아니면 전부 같은 분기를 탄다 —
   `'cut'`과 `'none'`을 시각적으로 구분해야 한다는 요구가 생기면(예:
   `'cut'`은 화면을 잠깐 검게 깜빡였다가 바뀌는 것처럼 다르게 보이고
   싶다면) 그때 분기를 나눈다. 지금은 두 값이 완전히 동일하게 동작한다.
4. **Auto Advance는 아직 미구현으로 남아있다.** Transition과 별개로
   착수해야 한다 — 타이머 기반 자동 진행이라 더 신중한 설계가 필요
   (TODO.md 참조).

## 9-36. CueList/"초기화" 버튼의 잔존 `confirm()` 제거 (2026-07-11, TODO에 없던 신규 발견)

### 배경

Auto Advance 보류 처리 이후 "TODO로 돌아가서 작업 진행"을 요청받아,
P0/P1/P2가 모두 소진된 상태에서 P3 중 유일하게 바로 착수 가능한 항목
("CueList 항목 텍스트 잘림 개선")을 시작하려고 `ui/CueList.js`를
읽었다. 그 과정에서 Page 삭제 버튼이 여전히 브라우저 네이티브
`confirm('이 Page를 삭제할까요?...')`을 쓰고 있는 걸 발견했다 — 9-32가
고쳤던 `prompt()` 취약점(반복 대화상자 차단 시 버튼이 안 먹는 것처럼
보이는 문제)과 정확히 같은 위험군인데, 9-32는 `prompt()`만 고치고
`confirm()`은 함께 안 고쳤던 것이다.

착수 전 사용자에게 발견 사실을 보고했고, "confirm(/prompt(/alert( 잔존
사용처를 전역 검색해줘. 더 없으면 이번에 한 번에 정리하자"는 요청을
받아 전체 저장소를 grep했다. 실제 호출부는 두 곳뿐이었다 —
`ui/CueList.js`(Page 삭제)와 `index.html`의 "초기화" 버튼(전체 Page
삭제). 나머지 매치는 전부 주석/문자열 설명이었다. 원래 계획했던
"CueList 항목 텍스트 잘림 개선"은 이 발견 때문에 이번 세션에서는
보류하고, 같은 위험군 버그부터 먼저 처리했다.

### 수정

**`ui/CueList.js`**: Song/Section/Media 삭제와 동일한 2단계 인라인
확인 패턴으로 교체했다. `pendingDeletePageId`/`pendingDeleteBtnEl`
모듈 스코프 변수로 대기 상태를 추적하고, 삭제 버튼을 두 번 눌러야
실제 `REMOVE_PAGE`가 실행된다. 추가로, 다른 Page를 선택(클릭)하면
대기 중이던 삭제 확인을 자동으로 취소하게 했다 — 9-31에서 고쳤던
"Song 삭제 2단계 확인 상태가 무관한 조작 이후에도 남아있던" 회귀와
같은 종류의 문제를 미리 막기 위함이다. Store 상태(pages/sections)가
바뀌어 CueList 전체가 다시 그려질 때도 이전 DOM 참조가 끊기므로
`render()`의 `pagesChanged` 분기에서 함께 리셋한다.

**`index.html`**: "초기화" 버튼(`#clear-btn`, 전체 Page 삭제)도 같은
2단계 확인으로 바꿨다. 다만 이 버튼은 모달 안에 있지 않고 상시 노출된
툴바 버튼이라, Song/Section 목록 모달처럼 "다시 열 때 리셋"할 자연스러운
계기가 없다 — 대신 3초 타임아웃으로 확인 대기 상태를 자동 취소하게
했다(새로 도입한 패턴). 첫 클릭 시 버튼 텍스트가 "정말 초기화?"로
바뀌고, 3초 안에 다시 누르지 않으면 원래 텍스트로 자동 복귀한다.

**`ui/cuelist.css`**: `.cue-delete-btn.is-confirming` 스타일을 여기에
추가했다(처음엔 실수로 `index.html`에 넣었다가, `.cue-delete-btn`의
소유권이 명백히 이 파일("Ownership: CueList UI 스타일")에 있다는 걸
깨닫고 옮겼다). 확인 대기 중에는 `.cue-item:hover`가 아니어도 항상
보이도록 `display: flex`를 강제한다 — 그렇지 않으면 첫 클릭 후 마우스가
항목 밖으로 나가는 순간 버튼이 사라져(`.cue-delete-btn`의 기본
`display: none`, hover 시에만 보임) 두 번째 클릭을 할 수 없는
상태가 된다.

### 회귀 테스트 추가

`index.test.js`에 새 `describe` 블록(4개) — `index.html`/
`ui/CueList.js` 코드(주석 제외)에 `confirm()` 호출이 없는지, 두 파일이
각각 `clearBtnPending`/`pendingDeletePageId` 2단계 확인 패턴을 쓰는지.
전역 grep으로 이 두 곳이 유일한 실사용처였음을 미리 확인해뒀으므로,
이 테스트만으로 재발을 충분히 감시할 수 있다.

`npm test` 51개 전부 통과(기존 47 + 이번 4).

### 브라우저 테스트 (Playwright, 자동화)

임시 스크립트로 검증 후 삭제(D-028 관행).

- CueList 삭제: 1클릭 시 "정말 삭제?"로 바뀌고 네이티브 dialog 미발생
- 다른 항목 선택 시 대기 중이던 삭제 확인이 자동 취소됨
- 2클릭으로 실제 삭제(네이티브 dialog 미발생)
- "초기화" 버튼: 1클릭 시 "정말 초기화?"로 바뀌고 네이티브 dialog 미발생
- 2클릭으로 전체 삭제 + 버튼 텍스트 원상 복구
- 3초 타임아웃 후 자동으로 확인 대기가 취소되고 삭제되지 않음
- 콘솔 에러 0건
...
`ui/CueList.js`(2단계 확인 로직), `index.html`("초기화" 버튼 2단계
확인), `ui/cuelist.css`(`.is-confirming` 스타일), `index.test.js`
(회귀 가드 4건 추가), `docs/TODO.md`(신규 항목 추가 및 완료 처리).

### 다음 단계 진입 시 주의사항

1. **원래 계획했던 "CueList 항목 텍스트 잘림 개선"(P3)은 아직
   미착수다.** 이번 세션은 그 작업 도중 발견한 별개 버그를 먼저
   처리한 것 — 다음 세션에서 원래 항목을 이어간다.
2. **"초기화" 버튼의 3초 타임아웃은 이번에 처음 도입한 패턴이다.**
   Song/Section/Media 삭제의 "모달 열 때 리셋" 패턴과 다르다 — 상시
   노출된 툴바 버튼에 한정해서만 쓴다. 앞으로 비슷한 상시 노출 버튼에
   2단계 확인이 필요해지면 이 패턴(타임아웃 자동 취소)을 재사용할 수
   있다.
3. **`confirm()`/`prompt()`/`alert()` 전역 grep 결과, 이제 실제 코드에
   남은 네이티브 블로킹 dialog 호출은 0건이다** — `index.test.js`의
   회귀 가드 4개(9-36)와 4개(9-32)가 이걸 계속 감시한다.

## 9-37. CueList 항목 텍스트 잘림 개선 (2026-07-11, TODO.md 로드맵의 P3 항목 구현)

### 배경

9-36(confirm() 제거)에 밀려 미뤄졌던 원래 작업을 이어서 진행했다.
`docs/TODO.md`에 "설계 방향 미정: hover 툴팁 vs 레이아웃 조정"으로
남아있던 항목이었다.

### 구현

`ui/CueList.js`의 `createCueItem()`에서 `.cue-preview` 요소에 `title`
속성을 추가했다 — `textContent`와 동일한 전체 텍스트를 담아, CSS
`text-overflow: ellipsis`(`ui/cuelist.css`)로 시각적으로 잘린 텍스트를
마우스 hover 시 브라우저 네이티브 툴팁으로 확인할 수 있게 한다.

레이아웃/CSS는 전혀 건드리지 않았다 — 커스텀 툴팁 컴포넌트를 새로
만드는 대신, 이미 이 프로젝트에 있던 패턴(Section 헤더의
`count.title = '이 섹션에 포함된 Page 수'`, `ui/CueList.js`)을 그대로
재사용했다. 텍스트 Page("가사첫줄")와 미디어 Page("가사첫줄 —
파일명") 모두 `getPreviewText(page)`가 만든 값을 그대로 쓰므로 별도
분기가 필요 없었다.

### 브라우저 테스트 (Playwright, 자동화)

임시 스크립트로 검증 후 삭제(D-028 관행). `node:test`로는 실제 CSS
잘림/hover 동작을 확인할 수 없어(D-028 범위 밖) 브라우저로 확인했다.

- 긴 텍스트 Page 생성 → `.cue-preview`의 `title` 속성에 전체 텍스트가 담김
- 실제로 `scrollWidth > clientWidth`로 시각적 잘림이 발생하는 상황임을 확인(툴팁이 실제로 필요한 케이스가 맞는지 전제 확인)
- 미디어 Page("가사첫줄 — 파일명" 조합)도 `title`에 전체 텍스트 포함
- 콘솔 에러 0건
...
`ui/CueList.js`(`.cue-preview`에 `title` 속성 추가), `docs/TODO.md`
(항목 완료 처리).

### 다음 단계 진입 시 주의사항

- 네이티브 `title` 툴팁은 표시까지 약 1초 지연이 있고 스타일을
  커스터마이징할 수 없다 — 실사용 중 이 방식이 불편하다는 피드백이
  나오면 커스텀 툴팁이나 레이아웃 자체 조정(예: 항목 높이를 2줄로
  늘려 줄바꿈 허용)으로 재검토한다. 지금은 가장 낮은 리스크의 해법을
  택했다.

## 9-38. 문서 정리 — D-019/D-020 예고 갱신 이행 + CurrentState.md 낡은 절 해소 (2026-07-11)

### 배경

9-37 이후 P0/P1/P2가 전부 소진된 상태에서 다음 작업 후보를 검토하던 중,
이전 검토(Sonnet)가 제안한 후보 3개를 재검증한 결과 두 개가 **이 문서의
낡은 기록 때문에 생긴 오판**임을 확인했다.

- "output.html 캐시 미스 정책 미정" 후보 → 근거였던 Step6 시절
  주의사항(9-33과 9-34 사이에 표류하던 블록)이 낡은 것이었다. 실제로는
  2026-06-27 합의로 결정·구현 완료(`output.html` 주석: resolve 완료까지
  구화면 유지, 로딩 UI 없음).
- "브라우저 테스트 부채 해소" 후보 → `ManualTestChecklist.md`의 "확인
  필요"는 현재 비어 있고, 초기 세션 부채는 이미 "과거 이력"으로 이관
  완료된 상태였다.

즉 문서 부패가 다음 세션의 판단을 실제로 오염시킨다는 것이 실증됐고,
마침 D-019/D-020이 "다음 문서 정리 세션에서 반영"으로 예고해 둔
아키텍처 문서 갱신도 미이행 상태로 남아있음을 grep으로 확인해
(CommandBusArchitecture.md/HistoryArchitecture.md에 async 직렬 큐/
bootstrapMediaCache 언급 전무), 두 부채를 한 세션에서 정리했다.
**코드는 무수정 — 문서만 변경했다.**

### 변경 내용

1. **`docs/CommandBusArchitecture.md`** — 말미에 "현재 구현 노트
   (D-018/D-019/D-020)" 절 추가. 본문(목표 구조)은 무수정으로 두고,
   현재 구현과의 차이만 기록: `execute()`의 async 전환 + 내부 직렬
   큐(본문 "Ordering Guarantee"의 실제 구현), `MEDIA_COMMANDS`
   whitelist 기반 `preloadMedia()`, 부팅 전용 `bootstrapMediaCache()`
   (dispatch/History 기록 없음 — Single Entry Point 원칙과 충돌하지
   않는 이유 포함), Hook이 범용 Registry가 아니라 HistoryManager 전용
   단일 슬롯이라는 사실(D-018). CommandExecutor/CommandRegistry/
   Composite/Transaction은 미구현(목표 구조)임을 명시.
2. **`docs/HistoryArchitecture.md`** — 같은 형식의 "현재 구현 노트
   (D-018/D-019)" 절 추가: `undo()`/`redo()`의 async 전환(await 완료
   후 `isApplyingHistory` 해제 — 재귀 기록 방지 타이밍), Entry 구조가
   본문의 Payload 기반이 아니라 완성된 Command 저장(`undoCommand`/
   `redoCommand`)인 이유(CommandRegistry 부재), 현재 Ignore 목록
   (SELECT_PAGE/CLEAR_SELECTION/GO_LIVE/CLEAR_LIVE/SET_APP_MODE).
3. **`docs/CurrentState.md`** — 낡은 절 3곳 정리: (a) 최종 업데이트
   헤더가 2026-07-03(9-12)에 멈춰 있던 것을 갱신, (b) 문서 상단
   "진행 중인 설계 변경(아직 미구현)" 절이 9-19에서 이미 구현 완료된
   D-Editor-4를 미구현이라 서술하던 것을 해소 표시, (c) Step6 시절
   주의사항 블록에 "1~4번 해소됨, 5번(GC)만 유효" 상태 주석 추가.
   원문은 당시 판단 기록으로 전부 보존 — 삭제가 아니라 상태 표시.

### 검증

- 문서 전용 변경이라 `npm test`/브라우저 테스트 대상 아님. 갱신 내용은
  `command/CommandBus.js`/`history/HistoryManager.js`/`domain/Page.js`
  (`isValidPage`)/`output.html`(resolve 정책 주석) 소스를 직접 읽고
  대조해 작성했다 — 문서가 코드보다 앞서가지 않도록 현재 코드에 실재
  하는 것만 기록.
- D-029 관점: Architecture 문서는 Tier 1이므로 본문(목표 구조)을
  수정하지 않고 말미에 "현재 구현 노트"를 분리 추가하는 방식을 택했다
  — 목표 구조와 현재 구현의 구분(D-029가 미해결로 남긴 "개별 문서의
  현재/Phase 2 여부" 문제)을 문서 안에서 명시적으로 드러내는 형식이다.

### 변경 파일

`docs/CommandBusArchitecture.md`, `docs/HistoryArchitecture.md`,
`docs/CurrentState.md`(헤더/상단 절/Step6 주의사항 + 이 절),
`docs/TODO.md`(항목 추가 및 완료 처리).

### 다음 단계 진입 시 주의사항

1. **신규 기능 착수 대상은 없다.** P3 항목들은 전부 트리거(실사용
   신호/Research 결론/선행 Decision) 대기가 맞다 — 다음 단계는 실사용
   관찰이다.
2. **D-019/D-020의 "영향 받는 문서" 항목은 이 세션으로 이행 완료됐다.**
   이후 Decision을 쓸 때 "다음 문서 정리 세션에서 반영" 방식의 이연을
   쓰면, 이번처럼 미이행 상태가 오래 남을 수 있다 — 가능하면 같은
   세션에서 문서까지 갱신하는 편이 안전하다.
3. **CurrentState.md의 "최종 업데이트" 헤더는 이제 최신 세션 번호를
   가리킨다.** 새 세션을 기록할 때 맨 아래에 절을 추가하면서 이 헤더도
   함께 갱신할 것 — 다시 방치되면 같은 문제가 재발한다.

## 9-39. 기술 부채 감사 + 선별 TODO 승격 (2026-07-11, 코드 무수정)

### 배경

P0~P2 소진 상태(9-38)에서 사용자 요청으로 전체 기술 부채 감사를
수행했다. 전 소스 파일 정독 + Decisions.md(D-001~D-029) 대조 + 테스트
목록 대조 방식이며, 결과는 `docs/Research/2026-07-11 Tech Debt Audit.md`
(Status: Research — 구현 근거 아님, D-029 순서 적용)에 기록했다.

### 핵심 발견

**TD-1(재현됨)**: `undo()` 완료 전 재호출 시 `isApplyingHistory`(단일
boolean)가 첫 undo의 finally에서 조기 해제되어, 두 번째 undo의 역방향
Command가 History에 재기록됨 — 임시 스크립트(작성→검증→삭제, D-028
관행)로 실제 모듈에서 재현: ADD_PAGE 2회 → 연속 undo 2회 결과가 기대
`undo 0/redo 2` 대신 `undo 1/redo 1`. state 자체는 정상(pages 정확히
복원), 오염되는 건 스택만. D-019가 고친 것과 같은 계열의 타이밍 버그로,
실사용 도달성은 낮다고 추정(큐가 media 캐시 미스 등으로 지연 중일 때
빠른 연속 Ctrl+Z 필요 — 브라우저 재현은 안 함, 추측임을 명시).

그 외: D-028 자동화 대상 계층(command/history/persistence/AppStore)
테스트 0건(TD-3), 전 UI가 D-017이 폐기 결정한 storeChanged 경로 사용
(TD-4), SET_SELECTION/SET_LIVE_PAGE마다 전체 저장(TD-5), test.html
금지 패턴 잔존(TD-6), docs/.obsidian 추적(TD-7) 등 — 전체 목록과
검증 수준 구분은 감사 보고서 참조.

### TODO 승격 판단 (사용자 요청: 전부 넣지 말고 선별)

- **승격 3건**: 핵심 계층 회귀 테스트(P1 — D-028 명시 미이행 + TD-1로
  필요성 실증), 연속 Undo 스택 오염 수정(P2 — 검증된 결함, 테스트
  선행 조건), 저장소 위생 test.html/.obsidian(P2 — 금지 패턴 잔존은
  9-38에서 실증된 "낡은 기록의 판단 오염"과 같은 계열).
- **비승격**: TD-2(추측 수준, 신호 없음), TD-4(Phase C/렌더링 최적화
  트리거 대기 — D-017은 문서 정합성 결정이었지 코드 이행 강제가 아님),
  TD-5(실사용 체감 없음, 주석상 의도된 동등 이전), TD-8(환경 문제,
  npm install 1회로 해소). 재검토 트리거는 감사 보고서에만 둔다.

### 변경 파일

`docs/Research/2026-07-11 Tech Debt Audit.md`(신규),
`docs/TODO.md`(헤더 + 로드맵 + Architecture TODO 3건),
`docs/CurrentState.md`(헤더 + 이 절). **코드 무수정** — 재현용 임시
스크립트는 검증 후 삭제했다.

### 다음 단계 진입 시 주의사항

1. **다음 착수는 P1(핵심 계층 회귀 테스트)이다.** TD-1 수정은 그 뒤 —
   순서를 뒤집으면 D-028이 경고한 "동작 고정 없이 타이밍 민감 코드
   수정"이 된다.
2. **착수 전 `npm install` 필요** — 현재 체크아웃에 node_modules가
   없어 `npm test`가 바로 안 돈다(TD-8).
3. **TD-1의 실사용 도달 조건은 추정이다** — 수정 시 브라우저 재현을
   시도해볼 수는 있으나, node 재현 시나리오가 이미 테스트로 고정될
   것이므로 필수는 아니다.

## 9-40. 장기 배포 전략 Research + 데이터 내보내기/가져오기 승격 (2026-07-11, 코드 무수정)

### 배경

사용자 요청으로 장기 배포 전략 Research를 작성하고 EXE 전환 필요성을
판단했다(구현 없음). 결과는 `docs/Research/2026-07-11 Deployment
Strategy.md`.

### 진행 중 전제 정정 (중요)

초판은 코드 이력(9-8 iPad 실사용 검증, 9-9/9-10 iOS 전용 버그 수정,
섹션 6의 "iPad/Safari — 주 사용 환경" 표기)에서 "주 사용 환경 =
iPad/Safari"로 역산해 "EXE는 방향 자체가 어긋남"으로 결론냈다. 그러나
사용자 확인 결과 **실운영 환경은 Windows/Mac 데스크톱·노트북 중심**
이었다 — iPad 이력은 테스트/검증이지 운영 표준이 아니다. **코드
이력만으로 운영 환경을 역산하면 틀린다**는 것이 이 세션의 교훈이며,
초판 원문은 보존하고(9-38 방식) 8절 재평가로 결론을 대체했다.

### 재평가 결론 (Research 8절)

- **웹 유지 + 운영 브라우저 Chromium 표준화**가 단기 최적 — 데스크톱
  전제에서 Window Management API/File System Access API/persist()까지
  웹인 채로 확보 가능해 상한이 오히려 올라간다.
- **PWA는 기본 경로에서 제외** — iPad 이점 소멸, EXE와 이중 투자.
- **EXE는 정합적인 장기 종착지 후보로 승격, 단 착수는 아직 아님** —
  실사용 마찰 보고 0건, 선행 부채(AppStore 동기 부팅 해체) 존재.
  착수 조건: 브라우저 운영 마찰 반복 확인 또는 비기술 운영자 배포
  필요. 착수 시 Electron 우선(D-009 런타임 고정 근거).
- **선택지 B(데이터 내보내기/가져오기 + storage.persist())가 전제
  변경과 무관한 최우선** — 브라우저 저장소 축출 시 전체 데이터 복구
  불가(백업 기능 부재)가 현 구조의 유일하게 "지금 아픈" 위험.

### TODO 승격

선택지 B를 P2로 승격(사용자 승인) — "데이터 내보내기/가져오기 +
저장소 persist". Decision 충돌 없음 확인(D-004 제외 규칙, D-016 경계,
D-025 version 관행 — Research 5절). EXE는 착수 조건 전까지 TODO에
올리지 않는다.

### 변경 파일

`docs/Research/2026-07-11 Deployment Strategy.md`(신규 + 재평가),
`docs/TODO.md`(헤더 + P2 로드맵 + Feature TODO 1건),
`docs/CurrentState.md`(헤더 + 이 절). 코드 무수정.

### 다음 단계 진입 시 주의사항

1. **P1(핵심 계층 회귀 테스트, 9-39)이 여전히 최우선이다.** 이번 승격
   항목(P2)은 CommandBus/History와 독립된 저장소 레벨 기능이라 순서
   제약은 없지만, 로드맵상 P1 먼저.
2. **가져오기(import)는 파괴적 동작이다** — 착수 시 2단계 확인 UX
   필수, 손상 파일은 migrateSnapshot+sanitizePresentation 경유로 기존
   데이터를 파괴하지 않고 거부해야 한다(TODO 항목 Completion Criteria
   참조).
3. **운영 환경 관련 과거 기록 주의** — 이 문서 섹션 6 등의 "iPad 주
...
## 9-42. 연속 Undo 시 History 스택 오염 수정 — TD-1 (2026-07-11, TODO P2)

### 수정

`history/HistoryManager.js`의 재귀 기록 방지 가드를 boolean 플래그
(`isApplyingHistory`)에서 **카운터(`applyingDepth`)**로 교체했다.
`afterExecute()`는 `applyingDepth > 0`이면 기록하지 않고, `undo()`/
`redo()`는 `++`/`finally --`로 감싼다. 공개 API `isHistoryApplying()`의
시그니처/의미는 유지(`> 0` 반환).

원인(감사 9-39에서 재현): undo()가 완료되기 전에 undo()가 다시 호출되면
(CommandBus 큐가 media preload 등으로 지연 중일 때 빠른 연속 Ctrl+Z),
첫 undo의 finally가 boolean 가드를 통째로 해제 → 아직 큐에서 대기
중이던 두 번째 undo의 역방향 Command가 가드 없이 dispatch되어
afterExecute에 재기록 → 스택 오염(기대 undo 0/redo 2 → 실제 1/1).
D-019가 고친 "한 undo 안에서의" 타이밍 버그와 같은 계열인 "두 undo
사이의" 버그다.

두 수정안(카운터 vs undo/redo 자체 직렬화) 중 카운터를 택했다 —
직렬화는 CommandBus의 직렬 큐와 이중 직렬화가 되어 과하고, 카운터는
겹친 replay가 전부 끝날 때까지 가드를 유지하는 최소 변경이다. 겹친
undo 2회의 redoStack 적재 순서도 순차 실행과 동일함을 확인(각 undo가
자기 execute의 resolve 후 push하고, resolve 순서는 큐 순서를 따르므로).

### 검증

9-41이 깔아둔 재현 테스트(`history/HistoryManager.test.js`의 "연속
undo 2회")에서 `{ todo: true }`를 제거해 정식 회귀 가드로 승격 —
**88 tests: 88 pass, 0 fail, 0 todo, exit 0.** 프로덕션 변경은
HistoryManager.js 한 파일이며, 기존 87개 테스트(직렬 큐/재귀 방지/
왕복 포함)가 전부 그대로 통과해 회귀 없음을 확인했다.

### 변경 파일

`history/HistoryManager.js`(가드 교체 + 주석 갱신),
`history/HistoryManager.test.js`(todo 표시 제거),
`docs/HistoryArchitecture.md`(현재 구현 노트의 가드 서술 갱신),
`docs/TODO.md`(항목 완료 처리), `docs/CurrentState.md`(헤더 + 이 절).

### 다음 단계 진입 시 주의사항

1. **남은 P2는 두 개다** — 저장소 위생(test.html/.obsidian, 즉시 가능)과
   데이터 내보내기/가져오기 + persist(9-40 승격). 아키텍처 리스크가
   없는 위생부터 처리하고 내보내기를 이어가는 순서가 자연스럽다.
...
## 9-48. 저장 트리거 정리(감사 TD-5) 사후 마감 (2026-07-13)

### 배경 — 9-46과 같은 부류의 사후 기록이다

D-031 커밋(9-47) 과정에서 작업 트리에 **커밋되지 않은 TD-5 코드**가
발견됐다 — `persistence/PersistenceSubscriber.js`의
`interestedMutations`에서 SET_SELECTION/SET_LIVE_PAGE 제거(주석의
날짜는 2026-07-12), `store/AppStore.js` 주석 갱신, 그리고
`persistence/PersistenceSubscriber.test.js`(신규 6건)까지 완성돼
있었으나 커밋/TODO 갱신/세션 기록이 전부 누락된 상태였다. 6cf1730
(9-46)에 이어 **마감 루틴 누락의 두 번째 사례**다. D-031 docs 커밋
(`9290535`)에 섞지 않고 분리해 이 세션에서 검증 후 마감했다.

### 확인 내용 (구현은 이 세션에서 하지 않음, 검증+마감만)

- 변경 자체는 TODO.md TD-5 항목의 계획 그대로: 저장 페이로드에
  selection/live가 애초에 없으므로(D-004) 두 Mutation 구독을 제거 —
  저장 내용/경로는 불변, 트리거만 감소(D-015 경계 불변).
- 테스트 6건이 고정하는 계약: interestedMutations 정적 가드(SET_PAGES/
  SET_SECTIONS/SET_TITLE만), 내용 변경 시 저장 실행, SELECT_PAGE/
  GO_LIVE 단독으로는 저장 0회, REMOVE_PAGE(3개 Mutation 동시 발생)
  시 저장 정확히 1회, subscriber 해제 정리.
- **전체 테스트 106/106 통과**(100 + 신규 6) — Windows node로
  UNC cwd에서 `node --test` 직접 실행으로 확인.
- UI/embedded script 무변경이므로 Playwright/ManualTestChecklist는
  해당 없음(D-028).

### 변경 파일

`persistence/PersistenceSubscriber.js`, `store/AppStore.js`(주석만),
`persistence/PersistenceSubscriber.test.js`(신규),
`docs/TODO.md`(TD-5 완료 처리 + 헤더), `docs/CurrentState.md`(이 절).

### 다음 단계 진입 시 주의사항

1. 9-46의 주의사항 2("TD-5 잔존")는 이 절로 해소됐다 — grep 당시엔
   실제로 잔존이었고, 그 뒤 커밋 없이 작업된 것.
2. 마감 루틴 누락이 두 번 반복됐으므로(6cf1730, TD-5) 세션 마감
   체크리스트를 CLAUDE.md에 명문화했다(9-49 예정) — 이후 세션은
   "코드 커밋에 문서 갱신 미동반 = 미완료"를 따를 것.

## 9-49. CLAUDE.md 감사·재작성 — 하위 모델 호환 (2026-07-13, 앱 코드 무수정)

### 배경

사용자가 claude.ai 채팅(Fable 5)에서 작성한 감사 프롬프트를 이
세션에서 실행했다. 목표: **하위 모델(Haiku 등)이 추가 맥락 없이 읽고
그대로 실행해도 틀리지 않는 CLAUDE.md**. 실태 파악(문서 + 코드/환경
대조 — 전체 테스트 실행 포함) → 감사 보고 → 변경 요약 사용자 승인(③
전부) → 재작성 순으로 진행.

### 감사에서 확인된 주요 사실

- 검증 절의 "현재 100건"은 실측 106건으로 이미 낡음 — 숫자 하드코딩
  제거.
- git wsl.exe 경유/이메일 오타 교정/UNC 경로 함정은 실무 관행인데
  미문서화 상태였음(이 세션에서도 리포 로컬 email이 오타로 돌아가
  있던 것을 커밋 전 교정한 사례 발생).
- 마감 루틴이 체크리스트로 없어 6cf1730(9-46)·TD-5(9-48) 누락이
  반복됨.
- fnr-patch 스킬이 채팅 시대 형식(FIND/REPLACE 블록 텍스트 출력)
  이었고 frontmatter가 YAML 펜스가 아니라 파싱이 깨져 있었음.
- (재작성 범위 외, 별도 제안으로 보류) 전역 CLAUDE.md의
  `@~/.claude/fable/active.md` import는 Windows 호스트 세션에서 해석
  안 되고, fable이 라우팅하는 에이전트(deep-reasoner/runner)는 WSL
  `~/.claude/agents`에만 있어 Windows 세션에 없음 — fable 시스템
  설계 판단이라 사용자 결정 대기.

### 변경 내용

- **CLAUDE.md(프로젝트)**: "세션 마감 순서 (필수)" 절 신설(5단계 +
  "문서 미동반 = 미완료"), "Git (함정 주의)" 절 신설(wsl.exe 경유 예시,
  커밋 전 email 확인·교정, 혼합 커밋 금지 — 기존 커밋 절 통합),
  개발 환경 절에 UNC/심링크 경로 함정 + Playwright 실행 예시 추가,
  "100건" → "fail 0 + 숫자 기준 금지", Skills 트리거를 객관 조건
  (폴더 목록)으로 교체. 나머지 절은 유지.
- **전역 CLAUDE.md**(`~/.claude/CLAUDE.md`, git 외부): "대형 변경"을
  "3개 이상 파일 또는 폴더 경계"로 수치화, 프로젝트 고유 파일명
  (TODO.md/CurrentState.md)을 "프로젝트 CLAUDE.md 규칙을 따르라"로
  위임.
- **.claude/skills/fnr-patch/SKILL.md**: frontmatter를 YAML 펜스로
  교정, FIND/REPLACE 블록 출력 형식을 Edit 도구 기준으로 재작성
  (의도인 "전체 재작성 금지, 최소 패치"는 유지).

### 변경 파일

`CLAUDE.md`, `.claude/skills/fnr-patch/SKILL.md`,
`docs/CurrentState.md`(이 절) + 리포 외부 `~/.claude/CLAUDE.md`.
앱 코드 무수정 — 테스트는 감사 중 실행한 106/106이 기준(9-48과 동일
시점).

### 다음 단계 진입 시 주의사항

1. 세션 마감 순서가 CLAUDE.md에 생겼다 — 코드 커밋 전 반드시 따를 것.
2. fable import/에이전트 배치 문제(M1/M2)는 미해결 보류 — Windows
   세션에서 fable 오케스트레이션 지시가 로드되지 않는 상태가 지속
   중이며, 해결하려면 (a) import 경로 양쪽 호환 또는 (b) agents를
   Windows `~/.claude/agents`에 복제 중 사용자 선택 필요.
   **(같은 날 후속)** 사용자가 (b)를 채택해 deep-reasoner/runner를
   `C:\Users\koor4\.claude\agents\`에 복제 완료(M2 해소, 다음 Windows
   세션부터 반영). M1(import 미해석)은 여전히 남음 — POSIX 절대경로
   + active.md 심링크(UNC ELOOP) 이중 원인이라, 완전 해결은 fable
   on/off 스위치를 심링크 대신 파일 복사 방식으로 바꾸는 수정이 필요.
   WSL/Windows 양쪽에 정의가 이원화됐으므로 fable agents 수정 시 양쪽
   동기화에 주의.

## 9-50. 스킬 감사 — FIX 1건 + 신규 2건 (2026-07-13, 앱 코드 무수정)

### 배경

사용자가 스킬 감사 프롬프트(사용 실태 → KEEP/FIX/MERGE/DELETE 분류 →
신규 제안 → 승인 후 실행)를 실행. 실태: 사용자 스킬은 프로젝트에
2개뿐(전역 스킬은 WSL/Windows 모두 없음). 분류 결과 KEEP=fnr-patch
(9-49에서 이미 재작성), FIX=architecture-review, MERGE/DELETE=해당
없음. 신규 제안 3건 중 사용자가 e2e-verify/session-closeout 2건 승인
(decision-record는 빈도 낮음으로 기각).

### 변경 내용

- **architecture-review 재작성(FIX)**: frontmatter YAML 펜스 교정
  (fnr-patch와 같은 파싱 깨짐), Step 1에 D-NNN 검색·충돌 시 중단
  (D-029) 의무화, 체크 항목을 실제 파일/결정(D-027/D-017/D-004,
  Schema.js)과 연결, "significant" → "3개 파일 이상 또는 레이어 경계"
  객관 조건, 마감 절은 CLAUDE.md 참조로 대체.
- **e2e-verify 신설**: Playwright 임시 E2E의 표준 레시피 스킬.
  템플릿은 이 세션에서 실제 실행해 PASS 확인 후 수록 — 핵심 함정
  2개를 명문화: ES Modules는 file://에서 차단되므로 내장 http 정적
  서버로 서빙, UNC 경로에서 `import.meta.url` 문자열 조작은 깨지므로
  `fileURLToPath()` 사용(둘 다 이 세션에서 실패→수정으로 검증).
  output.html은 same-origin 두 번째 페이지로 BroadcastChannel 공유.
- **session-closeout 신설**: CLAUDE.md "세션 마감 순서"의 실행 절차
  스킬(복붙 가능한 wsl.exe 명령 포함) — 작업 트리 전수 확인(의도 밖
  파일 발견 시 중단), 테스트, 문서 3종 대조, email 확인, 커밋 메시지
  파일(-F) 방식, 푸시 확인·보고의 6단계.
- **CLAUDE.md Skills 절**: 신규 2개의 트리거(객관 조건)를 추가.

### 변경 파일

`.claude/skills/architecture-review/SKILL.md`,
`.claude/skills/e2e-verify/SKILL.md`(신규),
`.claude/skills/session-closeout/SKILL.md`(신규), `CLAUDE.md`(Skills
절), `docs/CurrentState.md`(이 절). 앱 코드 무수정 — E2E 템플릿
검증용 `_e2e_tmp.mjs`는 실행 후 삭제(레시피대로).

### 검증

E2E 템플릿: 실제 실행 1회 PASS(부팅 + JS 에러 0건). 앱 코드 무수정
이므로 node --test 재실행은 생략(9-48의 106/106이 최신).

### 다음 단계 진입 시 주의사항

1. 이후 커밋이 걸린 작업은 session-closeout 스킬 절차를 따를 것 —
   특히 Step 1(작업 트리 전수 확인)은 생략 금지.
2. UI 검증은 e2e-verify 템플릿을 그대로 쓸 것 — 서버/경로 레시피를
   즉흥 변형하면 이 세션에서 밟은 함정(file:// 차단, UNC 경로)을
   다시 밟는다.

## 9-51. Live Emergency Overlay MVP 구현 (2026-07-13, D-031 이행)

### 배경

9-47에서 확정한 D-031 경계 안에서 마지막 P2를 구현했다. Observations
(2026-07-08) 요구사항 중 MVP 범위: 텍스트 오버레이, 상/중/하, 즉시
송출/해제. 출력 대상 선택(요구사항 7)은 D-031 Non-goal.

### 구현

- **domain/PresenterState.js**: `emergencyOverlay: null | { text,
  position }` 필드 + `setEmergencyOverlay()`/`clearEmergencyOverlay()`.
  도메인 수준 보장 2개 — 빈/공백 text는 해제로 처리(빈 오버레이가
  화면을 가리는 사고 방지), 허용값 밖 position은 'bottom' 폴백(잘못된
  값 때문에 긴급 송출이 실패하면 안 됨). 해제 no-op 시 같은 참조 반환.
- **store/AppStore.js**: SET/CLEAR_EMERGENCY_OVERLAY reducer 케이스
  (변경 없음이면 기존 state 반환 — 통지 생략), `deriveMutations()`에
  `SET_EMERGENCY_OVERLAY` Mutation(설정/해제 공용, 참조 비교).
- **history/HistoryManager.js**: 두 액션을 기존 Ignore 블록에 추가 —
  긴급 오버레이가 Ctrl+Z로 해제/재송출되는 건 운영 중 사고 경로.
- **output/BroadcastOutput.js**: `SHOW_OVERLAY`/`CLEAR_OVERLAY` 별도
  메시지(+`lastSentOverlay` 참조 dedupe — SHOW_PAGE의 dedupe와 분리),
  구독 Mutation에 SET_EMERGENCY_OVERLAY 추가(발생한 Mutation에 해당하는
  쪽만 전송), **REQUEST_SYNC 응답에 Overlay 상태 재전송 포함**.
- **output.html**: `#emergency-overlay`를 `#output-container`의
  **형제**로 배치 — renderPage()/showStandby()가 container.innerHTML을
  통째로 비우기 때문에, 컨테이너 안에 넣으면 매 렌더마다 지워진다.
  형제 + z-index 100으로 "CLEAR/슬라이드 전환이 오버레이를 안 건드림
  + STANDBY 위 렌더"(D-031 결정 4)가 구조적으로 보장된다. 오버레이
  메시지는 renderQueue를 타지 않는다(media resolve 같은 비동기 단계가
  없어 순서 역전 불가 + 즉시성 우선).
- **index.html**: 헤더 상시 노출 컨트롤(입력 + 상/중/하 select +
  긴급 송출/해제 버튼, Enter로도 송출). 클릭 핸들러는 execute()만
  호출하고, 버튼 표시 상태(is-broadcasting)는 SET_EMERGENCY_OVERLAY
  Mutation 구독이 갱신 — state가 유일한 출처.

### 변경 파일

`domain/PresenterState.js`, `domain/PresenterState.test.js`(신규),
`store/AppStore.js`, `store/AppStore.test.js`,
`history/HistoryManager.js`, `history/HistoryManager.test.js`,
`output/BroadcastOutput.js`, `output.html`, `index.html`,
`docs/TODO.md`, `docs/ManualTestChecklist.md`,
`docs/CurrentState.md`(이 절).

### 검증

- 회귀 테스트 10건 추가(PresenterState 6 + AppStore 4) + Ignore 정책
  테스트 확장 — **전체 116/116 통과**.
- Playwright E2E(e2e-verify 스킬 템플릿) **체크 11건 전부 통과 + JS
  에러 0건**: STANDBY 위 오버레이 표시(Live 없음), Live 송출과 병행
  유지, 위치 변경 즉시 반영, 해제 시 슬라이드 유지, **늦게 연 output
  창의 REQUEST_SYNC 오버레이/슬라이드 수신**, **Ctrl+Z에 오버레이
  미해제**, 컨트롤 is-broadcasting 토글. 스크립트는 검증 후 삭제.

### 다음 단계 진입 시 주의사항

1. **P0~P2 전부 소진** — 다음 착수는 P3 목록의 트리거/신호 충족
   여부를 먼저 확인한다(TODO.md "나중에" 절). 신호 없이 P3를 미리
   구현하지 않는다(Auto Advance 보류와 같은 YAGNI 원칙).
2. Overlay Engine(Countdown/QR 등)으로 확장하려면 D-031의 State 위치
   해석(요구사항 2 = "Presentation과 분리")부터 재검토 — D-031
   Non-goal 절 참조.
3. output.html의 오버레이 레이어는 container "형제"라는 구조에
   의존한다 — output.html 렌더링을 리팩터링할 때 오버레이를
   컨테이너 안으로 옮기면 CLEAR/전환마다 지워지는 회귀가 생긴다
   (E2E "해제 후 송출 슬라이드는 그대로 유지" 체크가 가드).

## 9-52. P2 두 기능 구현 — 가사 N줄씩 분할 + 미디어 썸네일 (2026-07-15)

### 배경

0714 관찰 ROI 평가(9-51 이후, TODO 승격 커밋 bd93648)에서 P2로 승격된
두 건을 사용자 승인 후 구현. 둘 다 고ROI(고가치·저비용)로 판단된 항목:
- #4 가사 자동 정렬/단락 구분(`Observations.md` 2026-07-14): 곡 가사를
  붙여넣을 때 빈 줄이 없어도 슬라이드 단위로 나누는 기능.
- #1a 미디어 라이브러리 썸네일: Library가 파일명만 보여줘 "어떤 파일인지
  모른다"는 직접 불편(직관성 부재) 해소.

설계 결정(사용자 확인): 단락 구분은 **N줄씩 균등 분할**(키워드 감지/
미리보기 확정 대신), 자동 정렬은 **공백 정리만**(가사 내용 무변경).

### 구현

**#4 (utils/lyricsImport.js — 순수 로직 확장):**
- `importLyrics(rawText, pageDefaults, options)`에 3번째 인자 추가.
  `options.linesPerSlide`가 양의 정수일 때만 균등 분할에 사용.
- 분절 규칙: **빈 줄이 있으면** 빈 줄 기준 분절(사용자가 의도적으로
  나눈 경계 존중, 기존 관습 유지) → linesPerSlide 무시. **빈 줄이
  전혀 없으면** linesPerSlide 줄씩 균등 분할(없으면 통짜 1개=기존 동작).
- 공백 정리(각 줄 trim + 빈 줄 제거)를 두 경로 모두에 적용.
- Page 생성 없는 순수 분절 함수 `splitLyrics()`를 export로 분리
  (테스트/향후 미리보기용). 하위 호환 유지(기존 호출/동작 불변).

**#4 (index.html — UI 연결):**
- editor-actions에 `#lines-per-slide` number input(기본 2) 추가,
  import-btn 핸들러에서 값을 읽어 세 번째 인자로 전달.
- lyrics-input placeholder에 "빈 줄이 없으면 설정한 줄 수마다 나눕니다"
  안내 추가.

**#1a (index.html — renderMediaList):**
- 각 미디어 row의 info 앞에 썸네일(img/video 52px, object-fit:cover)
  추가. blob은 `MediaStore.get()` 단건 조회(list()는 blob 제외 설계라
  그대로 두고, 항목별 lazy 조회) → `URL.createObjectURL`.
- 영상은 `<video muted preload=metadata>` + `#t=0.1`(첫 프레임 노출).
- **누수 방지**: kind별(image/video) objectURL 배열을 모듈 스코프에
  두고, renderMediaList 재호출 시 해당 kind의 이전 URL만 revoke.
  조회 실패(rec null) 시 `thumb.remove()`로 깨진 이미지 방지.
- MediaStore/도메인/store 무변경(index.html에서만 조회+렌더).

### 변경 파일

- `utils/lyricsImport.js` — splitLyrics export + importLyrics options
- `utils/lyricsImport.test.js`(신규) — 분절/정렬 회귀 14건
- `index.html` — 가사 N줄씩 UI + 핸들러 + placeholder, renderMediaList 썸네일
- `docs/ManualTestChecklist.md` — E2E 결과 한 줄

### 검증

- `node --test` 전체 130/130 통과(fail 0). 신규 14건(splitLyrics 빈 줄
  우선/균등 분할/공백 정리/하위 호환/경계값 + importLyrics Page 생성).
- Playwright E2E 9건(e2e-verify 임시 스크립트, 검증 후 삭제): 빈 줄 없는
  4줄+N=2 → Page 2개, 빈 줄 포함 시 빈 줄 우선(N 무시), 이미지=img·영상=
  video 썸네일에 blob URL, 콘솔 에러 0건.

### 다음 단계 진입 시 주의사항

1. `MediaStore.list()`는 여전히 blob을 제외한다(메모리 절약 설계) —
   썸네일은 항목별 `get()` lazy 조회에 의존한다. Library 항목이 매우
   많아지면 조회/objectURL 수가 늘어나므로, 성능 이슈 시 지연 로딩
   (IntersectionObserver)이나 썸네일 캐시를 검토한다(지금은 불필요).
2. 남은 P2 없음. 다음 후보는 P3(#5 개별 소스 라우팅·#3 text+media는
   Research/범위 정의 선행, #4의 "가사 추가 Song 통합"은 실사용 신호
   대기). TODO.md "나중에" 절의 트리거 충족 여부부터 확인한다.
3. 가사 분절 규칙을 더 확장할 때(절/후렴 키워드 감지, 미리보기 확정)는
   `splitLyrics()`가 순수 함수로 분리돼 있으므로 그 지점에 얹는다 —
   Reflow(`Observations.md` 2026-07-05)와 설계가 겹친다.

---

## 9-53. Output 창 자동 배치 (Window Management API) (2026-07-15)

### 배경

P0~P2 소진 후 남은 P3 중 사용자가 이 항목을 착수로 지정(보류 사유였던
"실사용 편의 신호"를 사용자 지시로 갈음). 기존 Output 버튼은
`window.open('./output.html', ..., 'width=1280,height=720')`로 고정 크기
창만 열어, 운영자가 부 모니터로 창을 **수동으로 끌어가야** 했다. Window
Management API(`getScreenDetails()`)로 비-primary 화면을 감지해 자동 배치.

**범위**: "어느 모니터에 배치하느냐"까지만. 출력별로 다른 내용을 보내는
개별 소스 라우팅(main/bus, TODO #5)과 Fullscreen API는 별개 항목이라 제외.

### 구현

`index.html` Output 클릭 핸들러(1곳)만 수정:
- `placeOnSecondaryScreen(win)` 헬퍼 추가 — `'getScreenDetails' in window`
  가드 → `await window.getScreenDetails()`(최초 호출 시 권한 프롬프트) →
  `screens.find(s => !s.isPrimary)` → 있으면 `win.moveTo(availLeft,
  availTop)` + `win.resizeTo(availWidth, availHeight)`.
- **설계 핵심(순서)**: 창을 사용자 제스처 안에서 **먼저** `window.open`
  으로 열고, 그 뒤 `placeOnSecondaryScreen()`을 fire-and-forget으로 호출.
  이유: `getScreenDetails()`의 권한 프롬프트가 `window.open`에 필요한
  제스처(transient activation)를 소모할 수 있어, 순서를 뒤집으면 팝업이
  차단될 위험이 있다. 먼저 열면 기존 고정 1280x720 창이 **항상** 보장돼
  폴백이 무손상이다.
- try/catch로 감싸 미지원/권한 거부/단일 모니터(secondary 없음)면 아무
  것도 하지 않고 기존 창 그대로 둔다.

BroadcastChannel(output.html 통신)/도메인/Store/Persistence 무수정.

### 변경 파일

- `index.html` — Output 클릭 핸들러에 placeOnSecondaryScreen 추가
- `docs/ManualTestChecklist.md` — 폴백 E2E 결과 + 실기기 미확인 항목
- `docs/TODO.md` — 항목 완료 압축 + 헤더
- (이 절)

### 검증

- `node --test` 전체 130/130 통과(fail 0) — index.html embedded script만
  건드려 테스트 대상 계층 무영향, 회귀 없음 확인.
- Playwright E2E 5건(e2e-verify 임시 스크립트, 검증 후 삭제): 헤드리스=
  단일 화면·권한 없음이라 **폴백 경로**가 실행됨. 팝업(output.html)
  열림·부팅, 버튼 "송출 중"/broadcasting 클래스 전환, 재클릭 시 중복 창
  미생성(기존 focus 분기 회귀 없음), JS 에러 0건.
- **미검증(정직 기록)**: 다중 모니터 실기기에서의 실제 배치(권한 프롬프트
  → moveTo/resizeTo)와 권한 거부 폴백은 헤드리스로 재현 불가라
  `ManualTestChecklist.md` "확인 필요"에 남겼다.

### 다음 단계 진입 시 주의사항

1. `getScreenDetails()`/`moveTo`/`resizeTo` 크로스-스크린 이동은
   **window-management 권한**이 있어야 동작한다. 권한 없으면 moveTo는
   현재 화면으로 클램프되므로, 폴백은 "부 모니터 이동이 안 될 뿐 창은
   정상"이다 — 이 동작을 바꾸지 말 것(무손상 폴백이 이 기능의 안전선).
2. 이 항목과 연계된 **모니터별 개별 소스 라우팅(main/bus, TODO #5)**은
   별개다 — Output 레이어 재설계 + 새 Decision 선행. 자동 배치가 그
   인프라의 일부처럼 보여도 여기서는 "배치"만 했다(내용은 여전히 단일
   BroadcastOutput이 모든 출력에 동일 송출).
