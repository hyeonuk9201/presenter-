...
Decision은 그 작업이 실제로 시작될 때 적용된다. 다만 `pageStyleDefaults`
(Section 단위 스타일 기본값)는 Song/Library와 무관하게 그 자체로 유용한
기능(Section 전체 글자 크기 일괄 변경 등)이라, Song/Library보다 먼저
별도로 구현해도 무방하다 — TODO.md에 그렇게 기록한다.
