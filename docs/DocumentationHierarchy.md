...
> **Status: Confirmed by `D-029`(2026-07-11)**
>
> 이 문서는 `docs/` 폴더의 각 문서가 실제로 어떤 역할을 하고 있는지,
> 그리고 문서끼리 내용이 어긋날 때 어떤 순서로 신뢰해야 하는지를
> 정리한 지도다. 2026-07-11 Draft로 커밋된 뒤 9-34/9-35 두 세션에서
> 실제로 적용해 마찰 없음을 확인, `D-029`로 승격했다(`Decisions.md`
> 참조).
>
> **단, 균일하게 확정된 게 아니다.** `D-029`가 강제로 승격한 건 딱
> 두 가지뿐이다 — ① `Decisions.md`만 충돌 판단의 유일한 기준이라는
> 것, ② 새 작업 착수 전 `Decisions.md` 확인이 항상 먼저라는 순서.
> 아래 Tier 1~5 분류와 "충돌 시 해석 순서"의 2~5단계는 구조는
> 유지하되 아직 실사용으로 검증되지 않은 **잠정 채택** 상태다 — 실제로
> 그 부분을 참조하는 세션이 생기면 `CurrentState.md`에 남겨 검증
> 사례를 쌓는다. 상세는 `D-029`의 "강제(검증됨)" vs "참고(잠정)" 구분
> 참조.
...
## 우선순위 계층 (Tier) — Tier 0만 `D-029`로 강제, 나머지는 잠정

아래 Tier 구분과 각 Tier 안의 상대 순서는 위 1~7번 근거를 바탕으로
이 문서가 **정리**한 것이다. Tier 사이의 "누가 이긴다"는 위에서
...
## 충돌 시 해석 순서 — 1단계만 `D-029`로 강제, 2~5단계는 잠정

문서 두 개가 서로 다른 말을 할 때 어떻게 판단할지 정리한다. 1단계
("Decisions.md부터 확인")는 9-34/9-35에서 실제로 쓰여 `D-029`로
강제 승격됐다. 2~5단계는 아직 실제 충돌 상황에서 쓰인 적이 없어
구조는 유지하되 관행 수준으로만 취급한다(`D-029` Non-goal 참조).
...
- ~~이 문서 자체를 `Decisions.md`에 D-0XX로 승격할지, 아니면 계속
  "참고용 지도" 상태로 둘지는 아직 결정하지 않았다.~~ → **해소
  (2026-07-11, `D-029`)**: 핵심 판단 순서(위 두 가지)만 강제로 승격,
  나머지 Tier 구조는 잠정 채택 상태로 유지하기로 결정.
