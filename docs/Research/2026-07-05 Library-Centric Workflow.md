...
- ~~같은 Asset을 여러 Presentation이 참조하는 상태에서, Asset을
  수정하면 이미 만들어진 Page들에도 변경이 전파돼야 하는가, 아니면
  배치 시점의 스냅샷을 유지해야 하는가?~~ → **`Decisions.md`의
  `D-021`(2026-07-06)로 해소**: 전파도 순수 스냅샷도 아닌 "당겨오기
  (pull)" 방식 — Section에 `sourceSongId`를 두고, 사용자가 명시적으로
  "다시 가져오기"를 실행하면 그 Section 전체를 교체한다. Page는 출처를
  모르고(Section만 안다), 자동 업데이트 감지도 만들지 않는다.
...
- Song Aggregate 자체(`songId`, Library 저장 스키마)를 지금 구현하는 것
  — `D-021`은 Song/Library가 실제로 만들어질 때 적용될 규칙을 미리
  정해둔 것일 뿐, Song/Library 자체의 구현 착수를 의미하지 않는다.

`TODO.md`의 "Asset/Song 관계 재검토" 항목이 실제로 착수될 때, 이
문서와 `Browser-Flow Separation.md`, `Decisions.md`의 `D-021`을 함께
다시 읽고 남은 Open Questions부터 마저 정리한다. `D-021`에 포함된
`pageStyleDefaults`(Section 단위 스타일 기본값)만은 Song/Library와
무관하게 먼저 구현 가능 — `TODO.md` 참조.
