...
| TD-4 | D-017 vs 이중 통지 병행 — 전 UI가 legacy 경로 사용 | 구조 부채 | 중간 | 확인됨 → **9-44에서 TODO P2 승격**(UI 통지 경로 이행) |
| TD-5 | 선택/송출 변경마다 전체 저장 | 성능 부채 | 중간 | 확인됨 → **9-44에서 TODO P3 승격**(저장 트리거 정리) |
| TD-6 | test.html — 금지 패턴 위반 레거시 파일 잔존 | 위생 | 낮음 | 확인됨 → **9-43 해소**(삭제) |
| TD-7 | docs/.obsidian/ 이 git 추적됨 | 위생 | 낮음 | 확인됨 → **9-43 해소**(gitignore) |
...
## ~~권장 착수 순서 (TODO.md 승격은 사용자 판단 대기)~~ → 판단 완료 (9-44 해소 표시)

이 절의 사용자 판단은 전부 끝났다 — 1~3번은 9-41/9-42/9-43에서 완료,
4번(TD-5)과 5번 중 TD-4는 9-44에서 사용자 승인으로 TODO.md에 승격
(각각 P3/P2, 7필드 작성됨), TD-2는 실사용 신호 대기로 비승격 유지.
이하 원문은 당시 권고의 근거 기록으로만 남긴다.
