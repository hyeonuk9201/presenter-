...
- 신규 Page 생성 시 기본 `sectionId`를 무엇으로 할지(미분류 vs 현재
  선택된 Section) — "나중에 이동으로 고칠 수 있으니 큰 문제 아님"으로
  잠정 결론났으나, 구현 착수 시 다시 확인.
- Section CRUD(`ADD_SECTION`/`REMOVE_SECTION`/`UPDATE_SECTION`)가 여전히
  `HistoryManager.js`의 `computeInverse()`에 케이스가 없어 Undo 기록
  대상이 아니다(D-Editor-1 시절부터의 기존 상태, 이번 변경과 무관하게
  남아있는 별개 이슈) — 이번 마이그레이션 범위에 포함할지는 별도 결정.
