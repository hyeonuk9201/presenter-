...
- **Transition** — 필드는 있으나 실제 애니메이션 미구현.
- **Auto Advance** — 필드는 있으나 실제 타이머 미구현.
- **스타일 프리셋 — 업데이트(덮어쓰기)** — 저리스크·저비용이나 구현
  범위를 "프리셋 정의 자체 덮어쓰기"로 한정해야 함(⚠, 아래 참조).
...
- [ ] **Transition**
  ...
  - Reason: `domain/Page.js`에 필드 뼈대만 있음(9-12), 실제 fade/cut 애니메이션이 없어 필드가 죽은 데이터임.
  - Impact: `domain/Page.js`(필드 이미 존재), `output/BroadcastOutput.js`, `output.html`(전환 애니메이션 렌더링), `ui/PreviewPanel.js` 일부 영향 가능.
  - Dependency: 없음, 단 애니메이션 구현 방식(CSS transition vs JS 타이머) 결정 필요.
  - Risk: 중간 — Core Philosophy("편집보다 송출 안정성 우선", `Architecture.md`)에 직결되는 송출 레이어라 성능/타이밍 검증 필요.
  - Completion Criteria: Live Mode에서 실제 fade/cut 전환이 시각적으로 확인되고 최소 1개 케이스 브라우저 테스트 통과.

- [ ] **Auto Advance**
  - Priority: P2 · Status: 예정
  - Reason: `domain/Page.js`에 필드 뼈대만 있음(9-12), 실제 자동 전환 타이머가 없어 필드가 죽은 데이터임.
  - Impact: `domain/Page.js`(필드), 새 타이머/스케줄러 로직(`ui/PreviewPanel.js` 또는 신규 모듈), `command/CommandBus.js` 액션 추가 가능성.
  - Dependency: 없음 — Transition과 독립 착수 가능하나 함께 설계하면 효율적(둘 다 송출 타이밍 레이어).
  - Risk: 중간 — 타이머 기반 자동 진행은 Live 송출 중 의도치 않은 시점에 전환될 위험이 있어 신중한 테스트 필요.
  - Completion Criteria: 지정된 시간 후 자동으로 다음 Page로 전환되는 동작이 Live Mode에서 확인됨.
...
- [ ] **스타일 프리셋 — 업데이트(덮어쓰기)** ⚠(범위 주의)
  - Priority: P2 · Status: 예정(필요성 미확인)
  - Reason: 지금은 항상 "새로 저장"만 가능해 비슷한 이름의 프리셋이 계속 늘어남 — 기존 프리셋 값을 덮어쓰고 싶은 경우를 지원 안 함.
  - Impact: `AppSettingsStore.js`(프리셋 CRUD에 update 추가), 프리셋 목록 모달 UI에 "덮어쓰기" 버튼 정도로 국소적.
  - Dependency: 없음.
  - Risk: 낮음 — **단, 구현 범위를 "프리셋 정의 자체 덮어쓰기"로 한정해야 한다.** "프리셋을 덮어쓰면 이미 적용됐던 Page들도 자동 갱신"으로 해석/구현되면 `D-023` Non-goal("스타일 변경 시 기존에 적용됐던 Page를 일괄 업데이트하는 기능 지원 안 함")과 정면 충돌한다. 구현 시 값 복사 원칙(`D-023`)을 재확인할 것.
  - Completion Criteria: 기존 프리셋을 선택해 현재 값으로 덮어쓰는 UI가 동작하고, 이미 적용된 Page들에는 아무 영향이 없음을 회귀 테스트로 확인.
...
| 스타일 프리셋 — 업데이트(덮어쓰기) | `D-023` Non-goal | "적용됐던 Page 일괄 업데이트"로 확대 해석되면 충돌 — "프리셋 정의 자체 덮어쓰기"로 범위를 좁혀야 안전 | 구현 범위를 명시적으로 좁혀서 설계 |
