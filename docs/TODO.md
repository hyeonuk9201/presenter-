...
>
> 최종 업데이트: 2026-07-11 (Transition 완료, 9-35 — P2 후보 중 리스크가
> 낮은 항목을 선정, output.html 크로스페이드로 구현하고 실제 설정 UI도
> 함께 추가. 이전: 스타일 프리셋 업데이트(덮어쓰기) 완료(9-34), Media
> Library UI 완료(9-33), 로드맵 구조 개편)
...
- **Auto Advance** — 필드는 있으나 실제 타이머 미구현. (Transition과
  "함께 설계하면 효율적"이라고 적어뒀었지만, 9-35에서 Transition만 먼저
  구현 완료 — Auto Advance는 타이머 기반 자동 진행이라 Live 송출 중
  의도치 않은 시점에 전환될 위험이 있어 범위를 분리해 더 신중히 접근.)

(스타일 프리셋 — 업데이트(덮어쓰기)는 9-34, Transition은 9-35에서 완료.
상세는 [Feature TODO](#feature-todo) 참조.)
