...
>
> 최종 업데이트: 2026-07-24 (output.html 송출 창 자체 캐시 blob URL 누수
> 해소 — D-0003 참조 기반 sweep, D-0001 Non-goal 후속. E2E 3건·전체
> 174/174 통과.)
> 이전 2026-07-24: main/bus 라우팅 Research 초안 작성 —
> `Research/2026-07-24 Output Identity-Routing Decision Draft.md`.
> 출력 identity=논리 feed(URL 파라미터, 무파라미터=main), 단일 채널+
> feed 필드 필터, 정의=AppSettingsStore·송출 상태=livePageIds 맵 권고.
> **미채택 — 사람 승인 대기**, 코드 무변경. 남은 쟁점 5건은 문서 참조.)
> 같은 날 이전: MediaRuntimeCache 참조 기반 sweep 구현 완료 —
> 0719 초안을 사용자 승인으로 채택, **`.itda/decisions.md` D-0001 등재**
> (ITDA 전환 후 첫 결정). domain/Presentation.js
> `collectReferencedMediaIds()` + MediaRuntimeCache `sweep(keepSet)` +
> CommandBus dispatch 후 pages 참조 비교 가드 3파일. 회귀 테스트 23건
> 추가(전체 166/166) + Playwright E2E 17건(삭제 시 축출·undo 재preload·
> D-032 배경 축출) 통과. blob URL 세션 잔존 누수 해소.
> 이전: 2026-07-19 blob 누수 evict 시점 Decision **초안** 작성 —
> `Research/2026-07-19 MediaRuntimeCache Evict Decision Draft.md`. 참조 기반
> sweep(CommandBus 내, domain 헬퍼로 keepSet 계산) 권고, 대안 4종 기각
> 근거·Non-goal·검증 계획 포함.
> 이전: TD-2 fire-and-forget 미처리 rejection 안전망
> 구현 완료, 9-55 — preloadMediaId가 IndexedDB 조회 실패를 "레코드 없음"과
> 동일 처리(execute reject/undo 이력 유실 차단) + index.html 전역
> unhandledrejection 토스트. 테스트 143/143 + E2E 2건. 같은 날 이전:
> 0716 탐색 승격 후보 2건 등록 — ① MediaRuntimeCache blob URL 누수(P3,
> evict 시점 Decision 선행 → 등록만, 미구현), ② TD-2(위에서 완료).
> 나머지 후보 4건(Reflow·자동 전체화면·IndexedDB GC·Session 통합)은
> 기존 문서의 보류/트리거 기록 유지로 충분해 미등록.
> 이전: 텍스트 Page 배경 미디어 구현 완료, 9-54 —
> D-032 배경형(A), text Page에 backgroundMediaId/backgroundMediaType
> 플랫 필드. domain/Page.js·view/PageView.js·command/CommandBus.js·
> ui/PreviewPanel.js·output.html·index.html 6파일, Mutation/History/
> 스키마 무변경(가법 필드). Page.test.js 신규 + 회귀, 전체 142/142 +
> E2E 9건. 자유 배치(B, Element)는 Non-goal. **남은 P3는 Decision
> 선행(다중선택·Song Undo·개별소스라우팅)/Research(Flow)/트리거 대기.**
> 이전: Output 창 자동 배치 구현 완료, 9-53 —
> index.html 클릭 핸들러에 placeOnSecondaryScreen() 추가, Window
> Management API로 부 모니터 감지 후 moveTo/resizeTo, 무손상 폴백.
> index.html만 수정(BroadcastChannel/도메인/Store 무관), E2E 폴백 5건.
> 이전: P2 두 건 구현 완료, 9-52 — 가사 N줄씩
...
- 감사의 나머지 항목 중 TD-8(node_modules)은 환경 문제라 **의도적으로
  승격하지 않았다**. TD-4는 9-46, TD-5는 9-48에서 완료. **TD-2(미처리
  rejection)는 당시 실사용 신호 부재로 비승격이었으나, 2026-07-16 탐색에서
  "Decision 없이 착수 가능한 유일 후보"로 재확인되어 2026-07-18에 P3로
  승격** — Architecture TODO의 해당 항목 참조.
...
- ~~**텍스트에 미디어 삽입 — 배경형(A)**~~ — 9-54에서 완료(D-032,
  backgroundMediaId 플랫 필드). 자유 배치(B, Element 모델)는 Non-goal로
  보류 — 상세는 [Feature TODO](#feature-todo) 참조.
...
- ~~**MediaRuntimeCache blob URL 미축출 누수**~~ — 2026-07-23 완료(D-0001
  참조 기반 sweep). 상세는 [Architecture TODO](#architecture-todo) 참조.
- ~~**output.html(송출 창) 자체 캐시 blob URL 미축출 누수**~~ — 2026-07-24
  완료(D-0003). D-0001이 "별도 realm·크로스페이드 revoke 위험"으로 미뤄둔
  Non-goal. output.html이 fill만 하고 revoke를 안 해 표시된 모든 mediaId가
  영구 누적되던 것을, container DOM에 붙은 슬라이드가 참조하는 mediaId만
  유지하는 참조 기반 sweep으로 해소(크로스페이드 중 겹친 두 슬라이드는 둘 다
  DOM에 있어 함께 keep).
- ~~**fire-and-forget 미처리 rejection 안전망(감사 TD-2)**~~ — 9-55에서
  완료. 상세는 [Architecture TODO](#architecture-todo) 참조.
...
- [x] **MediaRuntimeCache blob URL 미축출 — evict() 호출부 0곳 (런타임 누수)** — 2026-07-23에서 완료(D-0001)
  - Priority(당시): P3 · Status: 완료 · Reason: 2026-07-16 탐색에서 코드로
    확정(evict() 호출부 0곳 — 표시된 모든 미디어의 blob URL이 세션 종료까지
    잔존), D-032 배경·9-52 썸네일로 blob 생성 표면 증가 · Impact:
    `domain/Presentation.js`(`collectReferencedMediaIds()` 신설),
    `media/MediaRuntimeCache.js`(`sweep(keepSet)` 신설),
    `command/CommandBus.js`(dispatch+historyHook 후 pages 배열 참조 비교
    가드로 sweep — whitelist 아님) · Dependency: 축출 시점 Decision 선행 —
    초안(9-56) 작성 후 2026-07-23 사용자 승인으로 **`.itda/decisions.md`
    D-0001 등재**(ITDA 전환 후 첫 결정, 초안 문서는 설계 근거 원문으로
    유지)로 충족 · Risk: 중간이었으나 불변식("state.pages가 참조하는 id는
    keepSet에 항상 포함 → Live/Preview 참조 media는 구조적으로 축출 불가")
    + 2026-06-27 단일 쓰기 지점 원칙 확장(fill=preloadMedia, evict=sweep,
    둘 다 CommandBus 내부)으로 통제. output.html 자체 캐시는 2026-07-24
    D-0003으로 후속 해소, IndexedDB 영속 GC/LRU는 여전히 Non-goal(D-0001)
    · Completion Criteria: 회귀 테스트 23건
    (미참조 축출·공유 id 보존·backgroundMediaId 대칭·live+selected 보존·
    비-pages 명령 sweep 생략·undo 재preload·D-019 순서 회귀) 추가로 전체
    166/166 + Playwright E2E 17건(삭제 시 축출·잔존 이미지 유효 렌더·
    Ctrl+Z 재preload·D-032 배경 적용/제거 축출·JS 에러 0) 통과(충족,
    2026-07-23).

- [x] **fire-and-forget 미처리 rejection 안전망 (감사 TD-2)** — 9-55에서 완료
  - Priority(당시): P3 · Status: 완료 · Reason: 기술 부채 감사(9-39,
    TD-2) — `execute()` 호출부 전체가 fire-and-forget이라 IndexedDB 오류
    시 unhandled rejection + 조용한 실패, `HistoryManager.undo()`가 pop한
    entry 유실. 0716 탐색에서 "Decision 없이 착수 가능한 유일 후보"로
    재확인되어 승격 · Impact: `command/CommandBus.js`(`preloadMediaId`
    try/catch — IndexedDB 조회 실패를 "레코드 없음"과 동일 처리, 큐 주석
    정정), `index.html`(전역 `unhandledrejection` 핸들러 + showToast),
    `command/CommandBus.test.js`(회귀 1건 — IDBDatabase.prototype.transaction
    일시 패치로 조회 실패 재현) · Dependency: 없음 · Risk: 낮음(검증됨 —
    D-019 충돌 없음, preload 실패 시 dispatch 진행은 기존 "레코드 없음 →
    placeholder" 경로와 일관) · Completion Criteria: IDB 조회 reject에도
    execute가 resolve + dispatch 진행 + 캐시 미기록을 테스트로 고정(전체
    143/143) + 의도적 rejection 시 토스트 노출 E2E 통과(충족, 9-55).
...
- [x] **가사 자동 정렬 + paste 단락 자동 구분** — 9-52에서 완료
  - Priority(당시): P2 · Status: 완료 · Reason: `Observations.md`
    ...
    **→ 키워드 감지는 2026-07-23 실사용 신호로 별도 항목 승격·완료
    (D-0002, 바로 아래 항목).**

- [x] **가사 절 표시 마커 감지("1절"/"후렴" 경계 처리)** — 2026-07-23에서 완료(D-0002, 신규 발견 즉일 처리)
  - Priority(당시): P2(실사용 불편 직접 보고) · Status: 완료 · Reason:
    곡 편집 실사용에서 "1절" 같은 절 표시 줄이 N줄 자동 분할(9-52)에
    일반 가사 줄로 끼어 슬라이드 전체가 한 줄씩 밀림 — 사용자가 마커를
    손으로 지워야 했음. 9-52가 채택 안 하고 남겨둔 "절/후렴 키워드
    감지" 자리의 실사용 신호 · Impact: `utils/lyricsImport.js`
    (`isSectionMarker()` export 신설 — 마커 정의 단일 지점,
    `splitLyrics`를 마커 구간 분리 후 구간별 기존 규칙 적용으로 재구성),
    `index.html`(곡 편집기 `splitLyricsToBlocks` — 빈 줄 없을 때 마커
    기반 분리, isSectionMarker import 공유). 도메인/Store 무수정 ·
    Dependency: 설계 3택 사용자 확정(D-0002 — 마커=경계+제거, 한국어
    기본형만 보수적 감지, 두 경로 적용) · Risk: 낮음 — 오감지 시 가사
    줄이 사라지므로 패턴을 "줄 전체가 마커 단어"로 한정, 기존 테스트
    중 마커를 일반 내용으로 쓰던 3건은 규칙 변경에 맞춰 갱신 ·
    Completion Criteria: 마커 입력 시 밀림 없이 분할 + 마커 미표시,
    곡 편집기 빈 줄 없는 입력의 label/text 분리 + 왕복 복원, 빈 줄
    형식 하위 호환 유지(충족 — 테스트 8건 추가로 전체 174/174, E2E
    11건 + JS 에러 0).
...
- [ ] **모니터별 개별 소스 라우팅 (main/bus)**
  - Priority: P3 · Status: **Research 초안 작성됨(2026-07-24, 미채택 —
    사람 승인 대기)** — `Research/2026-07-24 Output Identity-Routing
    Decision Draft.md`. 승인 시 `.itda/decisions.md` D-0NNN 등재 →
    Phase 1(구조, 무설정 main 현행 동일) → Phase 2(운용 UI) 순.
...
- [x] **텍스트에 미디어 삽입 — 배경형(A)** — 9-54에서 완료(D-032)
  - Priority(당시): P3 · Status: 완료(범위 A만) · Reason: `Observations.md`
    2026-07-14 #3 — image/video Page에 text 오버레이는 되는데 **text
    Page에 미디어를 넣는 반대 방향**이 안 됐다는 사용자 보고(실수요
    확인). 사용자가 범위 A(배경형)로 착수 지정(9-54) · Impact:
    `domain/Page.js`(createTextPage에 backgroundMediaId/backgroundMediaType
    플랫 필드 + isValidPage 방어), `view/PageView.js`(createPageView
    3번째 인자 + text 브랜치 배경 레이어 — 기존 media 레이어 재사용),
    `command/CommandBus.js`(preloadMedia가 backgroundMediaId 병렬 조회),
    `ui/PreviewPanel.js`/`output.html`(배경 blob 해석 전달), `index.html`
    (라이브러리 "배경으로 적용" + 사이드바 배경 표시/제거) · Dependency:
    범위 결정(D-032로 A 확정) · Risk: 중간이었으나 기존 오버레이 렌더/
    UPDATE_PAGE/preload 패턴 재사용 + architecture-review로 통제 — Mutation/
    History/스키마 무변경(가법 옵셔널 필드, sanitize가 보존) · Completion
    Criteria: text Page에 배경 미디어+텍스트 동시 렌더(Preview/Output),
    배경 적용/제거 UI 동작 확인. domain/Page.test.js 신규 + CommandBus.test
    회귀, 전체 142/142. Playwright E2E 9건(적용→Preview/Output media+text
    레이어, blob URL, 사이드바 표시/제거) 통과(충족, 9-54).
  - **자유 배치(B, Element 모델)는 Non-goal(D-032)** — Element 모델 전환
    트리거("한 Page에 독립 편집 콘텐츠 3개 이상") 충족 시 재검토.
