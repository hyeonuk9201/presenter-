...
>
> 최종 업데이트: 2026-07-11 (배포 전략 Research + 데이터 내보내기/
> 가져오기 승격, 9-40 — `Research/2026-07-11 Deployment Strategy.md`
> 작성·재평가(운영 환경 = 데스크톱 중심으로 전제 정정, EXE는 장기
> 후보·착수 조건 대기), 선택지 B를 P2로 승격. 이전: 기술 부채 감사
> (9-39, 승격 3건 — 핵심 계층 회귀 테스트 P1, 연속 Undo 스택 오염
> 수정 P2, 저장소 위생 P2), 문서 정리(9-38))
...
- **핵심 계층 회귀 테스트 구축** — P1, 기술 부채 감사(9-39, TD-3)에서
  승격. D-028이 자동화 대상으로 명시한 command/history/persistence/
  AppStore 계층에 테스트가 0건인 채로 남아있고, 감사에서 바로 그
  계층(HistoryManager)의 잠재 버그가 실제 재현됨. 상세는
  [Architecture TODO](#architecture-todo) 참조.

### 다음 후보 (P2)

- **연속 Undo 시 History 스택 오염 수정** — 감사(TD-1)에서 재현된
  잠재 버그. 회귀 테스트(P1)가 선행 조건 — 상세는
  [Architecture TODO](#architecture-todo) 참조.
- **저장소 위생 — test.html 제거 + docs/.obsidian gitignore** —
  감사(TD-6/TD-7). 상세는 [Architecture TODO](#architecture-todo) 참조.
- **데이터 내보내기/가져오기 + 저장소 persist** — 배포 전략
  Research(9-40)에서 승격. 브라우저 저장소 축출 시 전체 데이터가 복구
  불가인 유일한 실위험의 해소책 — 상세는 [Feature TODO](#feature-todo)
  참조.
...
- [ ] **핵심 계층 회귀 테스트 구축 (CommandBus/HistoryManager/AppStore/Schema)**
  - Priority: P1 · Status: 예정
  - Reason: 기술 부채 감사(9-39, `Research/2026-07-11 Tech Debt Audit.md`
    TD-3)에서 승격. D-028이 자동화 대상으로 명시한 계층 중
    `command/CommandBus.js`(직렬 큐, MEDIA_COMMANDS preload,
    bootstrapMediaCache), `history/HistoryManager.js`(computeInverse 전
    케이스, 재귀 방지, Undo/Redo 왕복), `store/AppStore.js`(reduce 전
    액션, deriveMutations, markModifiedSongSections 예외),
    `persistence/Schema.js`(v1→v2 마이그레이션, sanitize 폴백)에
    테스트가 0건 — D-028의 핵심 근거("이 두 파일을 다시 만지기 전에
    회귀 테스트를 먼저")가 미이행이고, 감사에서 바로 그 파일의 잠재
    버그(아래 TD-1 항목)가 재현되면서 필요성이 실증됨.
  - Impact: `command/CommandBus.test.js`, `history/HistoryManager.test.js`,
    `store/AppStore.test.js`, `persistence/Schema.test.js`(전부 신규,
    co-locate — D-028 배치 규칙). 프로덕션 코드 무수정.
  - Dependency: 없음. (현재 체크아웃에 node_modules 미설치 — 착수 시
    `npm install` 1회 필요, 감사 TD-8.)
  - Risk: 낮음 — 테스트 추가만. 단, 현재 동작을 "고정"하는 것이
    목적이므로 테스트가 기대할 동작은 코드 현행 기준으로 작성한다
    (TD-1 버그 시나리오는 수정 전까지 알려진 실패로 명시).
  - Completion Criteria: 위 4개 영역의 회귀 테스트가 `npm test`로
    통과하고, TD-1 재현 시나리오(연속 undo 2회)가 테스트 코드에 포함됨.

- [ ] **연속 Undo 시 History 스택 오염 수정 (감사 TD-1)**
  - Priority: P2 · Status: 예정(회귀 테스트 P1 선행)
  - Reason: 기술 부채 감사(9-39)에서 실제 모듈로 재현된 잠재 버그 —
    `undo()` 완료 전 재호출 시 `isApplyingHistory`(단일 boolean)가 첫
    undo의 finally에서 조기 해제되어, 두 번째 undo의 역방향 Command가
    History에 재기록됨(기대 undo 0/redo 2 → 실제 undo 1/redo 1).
    D-019가 고친 것과 같은 계열의 타이밍 버그. 실사용 도달성은
    낮음(추정) — CommandBus 큐가 media 캐시 미스 등으로 지연 중일 때
    빠른 연속 Ctrl+Z 조건이 필요.
  - Impact: `history/HistoryManager.js`(플래그 → 카운터, 또는 undo/redo
    직렬화 중 택일).
  - Dependency: 핵심 계층 회귀 테스트 구축(P1)이 선행 — D-028 취지
    (타이밍 버그 이력이 있는 파일은 동작 고정 후 수정).
  - Risk: 중간 — D-019 이력이 있는 타이밍 민감 코드. 기존 Decision
    (D-018/D-019)과 충돌 없음은 감사에서 확인됨.
  - Completion Criteria: 재현 시나리오(연속 undo 2회)가 undo 0/redo 2로
    통과 + 기존 테스트 전부 통과 + `npm test`에 회귀 가드로 잔존.

- [ ] **저장소 위생 — test.html 제거 + docs/.obsidian gitignore (감사 TD-6/TD-7)**
  - Priority: P2 · Status: 예정
  - Reason: `test.html`(루트)은 초기 세션의 수동 테스트 하니스인데
    CommandBus를 우회하는 `dispatch()` 직접 호출을 담고 있다 —
    CLAUDE.md 금지 패턴에 해당하는 코드가 저장소에 잔존해, 새 세션이
    참고 패턴으로 오인할 위험(9-38에서 실증된 "낡은 기록이 다음 세션
    판단을 오염"과 같은 계열). `docs/.obsidian/`(특히 workspace.json)은
    Obsidian이 수시로 바꾸는 개인 상태 파일이라 diff 노이즈 유발.
  - Impact: `test.html` 삭제, `.gitignore`에 `docs/.obsidian/` 추가 +
    `git rm --cached`. 코드 로직 무수정.
  - Dependency: 없음.
  - Risk: 낮음 — test.html은 어떤 코드도 import하지 않는 독립 파일,
    D-028 이후 역할이 node:test + Playwright로 대체됨.
  - Completion Criteria: `test.html`이 저장소에 없고, `docs/.obsidian/`이
    미추적 상태이며, 앱 동작(index.html/output.html)에 영향 없음 확인.
