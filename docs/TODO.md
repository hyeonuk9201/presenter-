...
>
> 최종 업데이트: 2026-07-11 (연속 Undo 스택 오염 수정 완료, 9-42 —
> `isApplyingHistory` boolean을 `applyingDepth` 카운터로 교체, 알려진
> 실패였던 TD-1 테스트가 상시 가드로 승격되어 88 tests 전부 통과.
> 이전: 핵심 계층 회귀 테스트 구축(9-41, 테스트 4파일 신규), 배포 전략
...
- (연속 Undo 시 History 스택 오염 수정은 9-42에서 완료 — 아래
  Architecture TODO 참조.)
