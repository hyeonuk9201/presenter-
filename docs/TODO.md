...
- **MediaRuntimeCache blob URL 미축출 누수** — 2026-07-16 탐색에서 코드로
  확정(evict() 호출부 0곳). 축출 시점 Decision 선행 → 등록만, 구현 보류.
  상세는 [Architecture TODO](#architecture-todo) 참조.
...
- [ ] **MediaRuntimeCache blob URL 미축출 — evict() 호출부 0곳 (런타임 누수)**
  - Priority: P3 · Status: 보류(축출 시점 Decision 선행 + 실측 트리거 대기)
  - Reason: 2026-07-16 탐색 세션에서 코드로 확정 — `media/MediaRuntimeCache.js`의
    `evict()`가 export만 되고 전역 grep 결과 호출부가 0곳. 표시된 모든
    미디어의 blob URL(`URL.createObjectURL`)이 세션 종료까지 잔존한다.
    D-032 배경 미디어(9-54)·라이브러리 썸네일(9-52)로 blob 생성 표면이
    늘어 새 근거가 생겼다. 단 장시간·다미디어 세션에서만 체감되는
    문제로, 실측 통증 신호(메모리 증가/jank)는 아직 없음.
  - Impact: `media/MediaRuntimeCache.js`(evict 호출 설계),
    `command/CommandBus.js`(축출 시점 후보 — REMOVE_PAGE/UPDATE_PAGE 등),
    캐시 쓰기 단일 지점 원칙(D-019) 경계.
  - Dependency: **"언제 evict하나"에 대한 소규모 Decision 선행** —
    Live/Preview가 아직 참조 중인 blob URL을 회수하면 송출 화면이 깨지므로
    안전한 축출 시점 설계가 필요(D-019의 GC-Phase2 보류 경계에 걸림).
    IndexedDB 영속 GC(Library-Centric Research, D-002 재개 필요)와는 분리
    — 이 항목은 런타임 메모리만 다룬다. **Decision 초안 작성됨(2026-07-19,
    9-56)**: `Research/2026-07-19 MediaRuntimeCache Evict Decision Draft.md`
    — 참조 기반 sweep 권고(불변식: state.pages 미참조 id만 축출). 사용자
    승인으로 Decisions.md 등재 시 이 Dependency가 충족되어 착수 가능.
  - Risk: 중간 — 잘못 축출하면 Live 송출 중인 blob URL이 무효화되어 송출
    화면이 깨질 수 있음(Core Philosophy 직결). 방치 리스크는 낮음(메모리
    누적뿐).
  - Completion Criteria: 축출 시점 Decision(D-0XX) 확정 → 구현 +
    "Live/Preview가 참조 중인 media는 축출되지 않는다" 회귀 테스트 통과.
    재검토 트리거: 장시간 세션에서 메모리 증가/jank 실측 시 P2 승격.
