...
>
> 최종 업데이트: 2026-07-23 (가사 절 표시 마커 감지 구현 완료, D-0002 —
> 실사용 신호("1절" 줄이 N줄 분할에 끼어 슬라이드가 밀림)로 9-52가
> 남겨둔 자리를 이행. `isSectionMarker()` 신설(한국어 기본형만, 보수적
> 감지), 가사 추가=경계+제거·카운트 리셋, 곡 편집기=빈 줄 없을 때
> 마커→LyricBlock label. utils/lyricsImport.js + index.html 2파일.
> 테스트 8건 추가(전체 174/174) + E2E 11건 통과.
