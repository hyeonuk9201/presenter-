...
(9-45 완료) → ~~UI 통지 이행~~(9-46에서 완료 확인) → Emergency
Overlay(~~Decision 선행~~ → D-031 확정, 9-47 — 구현만 남음).

- **Live Emergency Overlay (MVP)** — Research 탐구(9-44)에서 승격.
  `Observations.md` 2026-07-08에 요구사항 7개가 이미 정리됨. 선행
  Decision은 **D-031로 확정(2026-07-13, 9-47)** — 상세는
  [Feature TODO](#feature-todo) 참조. UI 통지 이행도 9-46에서 완료
  확인됨 — Overlay 구독은 처음부터 `interestedMutations` 경로로
  설계하면 된다.
...
- [ ] **Live Emergency Overlay (MVP)**
  - Priority: P2 · Status: 예정 — **선행 Decision 충족(D-031, 9-47) — 바로 착수 가능**
  ...
  - Impact: 새 Overlay State(위치 미정 — Presentation/Page가 아님,
    ...
    가능성.
  - Dependency: ~~새 Decision 선행 필수~~ → **`D-031`로 확정(2026-07-13,
    9-47)** — PresenterState 필드(`emergencyOverlay`) + CommandBus 경유
    (History Ignore) + 별도 Broadcast 메시지(REQUEST_SYNC 재전송 포함) +
    livePageId 직교(STANDBY 위에도 렌더) + 출력 대상 선택(요구사항 7)은
    Non-goal. 구현은 D-031 경계 안에서 진행한다. UI 통지 이행(TD-4)도
    9-46에서 완료 확인됨 — 구독은 처음부터 `interestedMutations`로.
  - Risk: 중간 — Live 송출 경로에 새 렌더 레이어를 얹는 작업이라
    ...
    제외).
  - Completion Criteria: (1) Overlay State Decision 확정(충족 — D-031,
    9-47), (2) Live 중
