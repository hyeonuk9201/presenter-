...
>
> 최종 업데이트: 2026-07-18 (2026-07-16 탐색 세션의 승격 후보 2건 등록 —
> ① MediaRuntimeCache blob URL 미축출 누수(P3, evict 시점 Decision 선행
> → 등록만), ② fire-and-forget 미처리 rejection 안전망(감사 TD-2, P3,
> Decision 불필요 → 즉시 착수). 나머지 후보 4건(Reflow·자동 전체화면·
> IndexedDB GC·Session 통합)은 기존 문서의 보류/트리거 기록 유지로 충분해
> 미등록. 이전: 텍스트 Page 배경 미디어 구현 완료, 9-54 —
...
- **fire-and-forget 미처리 rejection 안전망(감사 TD-2)** — 2026-07-16
  탐색에서 "Decision 없이 착수 가능한 유일 후보"로 재확인, 2026-07-18
  착수. 상세는 [Architecture TODO](#architecture-todo) 참조.
...
- [ ] **fire-and-forget 미처리 rejection 안전망 (감사 TD-2)**
  - Priority: P3 · Status: 예정(2026-07-18 착수)
  - Reason: 기술 부채 감사(9-39, TD-2) — `execute()` 호출부 전체(index.html,
    ui/CueList.js)가 fire-and-forget이라 IndexedDB 오류 시 unhandled
    rejection + 사용자에게 조용한 실패. 부수 경로로 `HistoryManager.undo()`가
    pop한 entry가 어느 스택에도 남지 않고 유실된다. 당시엔 실사용 신호
    부재로 비승격이었으나, 2026-07-16 탐색에서 "Decision 없이 저비용으로
    착수 가능한 유일 후보"로 재확인.
  - Impact: `command/CommandBus.js`(`preloadMediaId`의 IndexedDB 오류를
    기존 "레코드 없음" 경로와 동일 처리 — warn 후 진행), `index.html`(전역
    `unhandledrejection` 핸들러 + 기존 showToast 재사용),
    `command/CommandBus.test.js`(회귀). 도메인/Store/History 무수정.
  - Dependency: 없음.
  - Risk: 낮음 — 큐 체인은 이미 `.catch`로 보호(D-019)되어 후속 Command가
    막히지 않으며, 이번 변경은 "조용한 실패"를 "관측 가능한 실패"로 바꾸는
    방어층. 감사 문서에서 기존 D-0XX 충돌 없음 확인됨. preload 실패 시
    dispatch를 진행하는 선택은 기존 "레코드 없음 → placeholder 렌더"
    경로와 일관(undo 주입 INSERT_PAGE_AT의 이력 유실도 함께 차단).
  - Completion Criteria: preload 중 IndexedDB 오류에도 execute가 reject하지
    않고 dispatch가 진행됨을 회귀 테스트로 고정 + unhandledrejection 발생
    시 토스트 노출 E2E + 전체 테스트 통과.
