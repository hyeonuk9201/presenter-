...
>
> 최종 업데이트: 2026-07-14 (Observations.md 0714 관찰 8건 ROI 평가
...
- **가사 자동 정렬 + paste 단락 자동 구분** — `Observations.md`
  2026-07-14 #4에서 승격(ROI ★★★, 고가치·저비용). `importLyrics()`
  가 이미 `split(/\n\s*\n/)` 순수 함수라 도메인/Store 무변경으로
  분절 로직만 개선 가능 — 상세는 [Feature TODO](#feature-todo) 참조.
- **미디어 라이브러리 썸네일/preview** — `Observations.md` 2026-07-14
  #1a에서 승격(ROI ★★★). "제목만으로 어떤 파일인지 몰라 작업량이
  는다"는 직접 불편. Blob이 이미 MediaStore에 있어 Library UI에
  preview만 얹으면 됨(도메인 무관) — 상세는 [Feature TODO](#feature-todo)
  참조. #1의 전체 flow 3분할 재구조화(#1b/#2)는 P3 Research로 분리.
- (2026-07-11 Research 탐구가 승격한 P2 세 건은 전부 완료됨:
...
- **Output 창 자동 배치(Window Management API)** — `Observations.md`
  2026-07-11 관찰에서 P3 승격. `window.open`을 부 모니터에 자동
  배치하도록 확장. 관찰이 스스로 "실사용 편의성 확인 시 착수"로
  못박은 트리거 대기 항목 — 상세는 [Feature TODO](#feature-todo) 참조.
...
- [ ] **Output 창 자동 배치 (Window Management API)**
  - Priority: P3 · Status: 보류(실사용 편의 신호 대기)
  - Reason: `Research/Observations.md`(2026-07-11 조사)에서 승격.
    현재 `index.html`의 Output 버튼은 `window.open('./output.html',
    ..., 'width=1280,height=720')`로 고정 크기 창만 열어, 운영자가
    부 모니터로 **수동으로 끌어가야** 한다. Window Management API
    (`getScreenDetails()`, W3C 표준, Chrome 100+)를 쓰면 연결된
    디스플레이를 감지해 output.html을 부 모니터에 자동 전체화면으로
    열 수 있다. 관찰 결과 이건 **웹 기술 한계가 아니라 설계 선택**임이
    확인됐고, `OutputArchitecture.md`의 `WindowOutput`(향후 슬롯)이
    받을 자리다. **범위 주의**: 이 항목은 "창을 어느 모니터에 여느냐
    (배치)"만 다룬다 — 출력별로 서로 다른 내용을 송출하는 "모니터별
    개별 소스 라우팅(main/bus)"(`Observations.md` 2026-07-14 #5)은
    별개의 훨씬 큰 항목이며 여기 포함하지 않는다.
  - Impact: `index.html`(Output 버튼 클릭 핸들러 — `window.open`
    호출부만 교체, `left`/`top`/`width`/`height`를 감지된 부 모니터
    `availLeft/Top/Width/Height`로 지정). BroadcastChannel 통신 구조
    (output.html 메시지 교환)는 **무수정**. 도메인/Store/Persistence
    무관. 문서상 `OutputArchitecture.md` WindowOutput 항목 구체화.
  - Dependency: 없음(기술적 선행 조건 없음). 단 **실사용에서 "매번
    수동으로 창 옮기는 게 불편하다"는 구체적 신호 확인이 착수 트리거**
    — 신호 없이 착수하지 않는다(Auto Advance 보류와 같은 YAGNI 판단).
  - Risk: 낮음~중간 — Decision 충돌 없음(WindowOutput은 이미 예고된
    향후 슬롯, D-017 구독모델과 무관). 제약 두 가지: (1) `getScreen
    Details()`는 **사용자 권한 허용**이 필요하고, (2) `window.open`은
    **사용자 제스처(클릭) 안에서만** 호출 가능 — 현재 Output 버튼
    클릭 핸들러가 이미 그 조건을 만족하므로 자연스럽게 얹힌다.
    API 미지원/권한 거부 시 **기존 고정 창 방식으로 폴백**해야 한다
    (기능 자체는 지금도 동작하므로 회귀 위험 없음).
  - Completion Criteria: 다중 모니터 환경에서 Output 버튼 클릭 시
    output.html이 부 모니터에 자동 배치됨 + 단일 모니터/권한 거부 시
    기존 방식으로 무손상 폴백 확인. UI 변경이므로 Playwright E2E로
    폴백 경로(단일 화면) 검증 + `ManualTestChecklist.md` 기록.

- [ ] **가사 자동 정렬 + paste 단락 자동 구분**
  - Priority: P2 · Status: 예정
  - Reason: `Observations.md` 2026-07-14 #4 승격(ROI ★★★). Song
    Library가 생기면서(9-28~30) presenter 초기 아이디어의 "곡 가사
    자동 정렬 + paste 가사 단락 구분"을 실현할 환경이 갖춰졌다는 사용자
    판단. 현재 `utils/lyricsImport.js`는 `split(/\n\s*\n/)`(빈 줄 기준)
    한 줄 분절뿐이라, 빈 줄 없이 붙여넣은 가사나 들쭉날쭉한 정렬은
    처리 못 한다.
  - Impact: `utils/lyricsImport.js`(분절/정렬 로직 확장 — 순수 함수),
    필요 시 `domain/*` 무변경. Song 생성 경로(`importSongAsSection`)와
    병존/연계 가능. 저비용의 핵심 근거: 이미 순수 함수라 도메인/Store/
    Persistence를 건드리지 않는다.
  - Dependency: 없음. 단, "가사 추가" 경로의 Song 통합(P3)과 설계가
    겹치므로 착수 시 함께 검토(원본 텍스트 보존 여부).
  - Risk: 낮음~중간 — 자동 분절 규칙이 사용자 의도와 어긋나면 오히려
    수동 교정이 늘 수 있어, "자동 제안 + 수동 확정" 방향이 안전.
    Decision 충돌 없음(`D-026`/`D-027` 경계 안, 순수 유틸).
  - Completion Criteria: 빈 줄 없는 paste 가사도 단락(슬라이드) 단위로
    자동 분절되고, 결과를 사용자가 확정/교정할 수 있음. 분절 규칙
    회귀 테스트(`utils/*.test.js`) + UI 경로 E2E.

- [ ] **미디어 라이브러리 썸네일/preview**
  - Priority: P2 · Status: 예정
  - Reason: `Observations.md` 2026-07-14 #1a 승격(ROI ★★★). Media
    Library UI(9-33)는 이미지/영상을 **제목(파일명)만으로** 나열해,
    "어떤 파일인지 몰라 작업량이 늘어난다"는 직접 불편(직관성 부재).
  - Impact: `index.html`(Library 모달 Backgrounds/Videos 탭 항목에
    썸네일/미리보기 렌더), 필요 시 `media/MediaStore.js`(썸네일 조회
    헬퍼). 저비용 근거: Blob이 이미 IndexedDB(MediaStore)에 있어
    도메인/Store 스키마 변경 없이 조회+렌더만 추가. 영상은 첫 프레임
    캡처 또는 `<video>` poster 방식 중 택일.
  - Dependency: 없음. #1 전체 flow 3분할 재구조화(#1b/#2, P3 Research)
    에서 **썸네일만 분리 승격** — 재구조화 없이 독립적으로 가치 발생.
  - Risk: 낮음~중간 — 대용량 미디어 다수일 때 썸네일 렌더 성능(지연
    로딩/캐시 고려). 순수 UI라 Decision 충돌 없음.
  - Completion Criteria: Library의 이미지/영상 항목에 썸네일이 표시되어
    파일명 없이도 식별 가능. UI 변경이므로 Playwright E2E + `Manual
    TestChecklist.md` 기록.
