...
>
> 최종 업데이트: 2026-07-11 (핵심 계층 회귀 테스트 구축 완료, 9-41 —
> CommandBus/HistoryManager/AppStore/Schema 신규 테스트 4파일, 88 tests
> 중 87 pass + TD-1 알려진 실패 1 todo. npm install로 TD-8도 해소.
> 다음 착수 후보는 연속 Undo 수정(P2, 선행 조건 충족). 이전: 배포 전략
> Research + 데이터 내보내기/가져오기 P2 승격(9-40), 기술 부채 감사
> (9-39), 문서 정리(9-38))
...
- **연속 Undo 시 History 스택 오염 수정** — 감사(TD-1)에서 재현된
  잠재 버그. 선행 조건이었던 회귀 테스트는 9-41에서 완료 — **착수
  가능**. 상세는 [Architecture TODO](#architecture-todo) 참조.
...
- [ ] **연속 Undo 시 History 스택 오염 수정 (감사 TD-1)**
  ...
  - Dependency: 핵심 계층 회귀 테스트 구축(P1)이 선행 — D-028 취지
    (타이밍 버그 이력이 있는 파일은 동작 고정 후 수정). **충족됨
    (9-41)** — `history/HistoryManager.test.js`에 TD-1 재현 시나리오가
    `{ todo: true }` 알려진 실패로 이미 포함되어 있어, 수정 후 todo
    표시만 제거하면 곧바로 회귀 가드가 된다.
