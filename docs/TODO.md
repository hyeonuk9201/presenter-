...
>
> 최종 업데이트: 2026-07-11 (Section 추가/스타일 프리셋의 `prompt()` 취약점 근본 해결, 9-32)
...
- [x] 테스트 인프라 도입 (node:test 기반) — `D-028` 참조, 2026-07-10 완료. `package.json`(`node --test` + `fake-indexeddb`) + `.gitignore` 구성, 첫 회귀 테스트(`domain/Song.test.js`, 14개 케이스) 작성, `docs/ManualTestChecklist.md` 신설로 브라우저 확인 부채를 한 곳에 모음. domain/store 전체 회귀 테스트 구축 자체는 "완료" 개념이 아니라 이제부터 새 기능/버그 수정 시마다 테스트를 같이 추가하는 지속적 관행으로 이어간다.
...
- [x] Section 추가 버튼의 `prompt()` 취약점 — **(2026-07-06 실사용 중 발견, 9-32에서 근본 해결)** 브라우저가 `prompt()`/`alert()`를 반복 호출 감지하면 "이 페이지가 추가 대화상자를 만들지 못하게 차단" 체크박스를 띄우는데, 실수로 체크하면 그 이후 프롬프트 자체가 안 뜨고 즉시 `null`을 반환해 버튼이 안 먹는 것처럼 보였다. 부분 대응(9-20, 9-21, 토스트 안내/자동 배정)에 이어, 9-32에서 `showTextPrompt()`(인앱 모달)로 교체해 Section 추가/스타일 프리셋 저장 두 호출부 모두 `prompt()` 자체를 제거 — 근본 원인 해소.
